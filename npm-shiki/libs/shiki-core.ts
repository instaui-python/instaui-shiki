...
async function createHighlighter(options: {
  ...
  const { themes, themeModulePath = "@shiki/themes/", langModulePath = "@shiki/langs/" } = options;

  const themesModule = await Promise.all(
    themes.map((theme) => import(/* @vite-ignore */ `${themeModulePath}${theme}.mjs`)),
  );
  ...
  async function codeToHtml(code: string, options: any) {
    const { lang } = options;
    if (lang) {
      await highlighter.loadLanguage(import(/* @vite-ignore */ `${langModulePath}${lang}.mjs`));
    }
