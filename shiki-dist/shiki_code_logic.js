...
function wC() {
  ...
  return {
    parse(g) {
      ...
      let C = 0;
      do {
        const I = IC(g, C), w = I.sequence ? g.substring(C, I.startPosition) : g.substring(C);
...
function eC(Q, A, B) {
  ...
  return E.map(
    (w) => I.parse(w[0]).map((D) => {
      let o, s;
      D.decorations.has("reverse") ? (o = D.background ? C.value(D.background) : Q.bg, s = D.foreground ? C.value(D.foreground) : Q.fg) : (o = D.foreground ? C.value(D.foreground) : Q.fg, s = D.background ? C.value(D.background) : void 0), o = T(o, g), s = T(s, g), D.decorations.has("dim") && (o = iC(o));
      let e = y.None;
      ...
      if (E.includeExplanation) {
        ...
        for (; J + nB < U; ) {
          ...
          nB += X.length, SA.explanation.push({
            content: X,
            scopes: E.includeExplanation === "scopeName" ? nC(
              q.scopes
            ) : aC(
              cB,
              q.scopes
...
function aC(Q, A) {
  ...
  for (let g = 0, E = A.length; g < E; g++) {
    ...
    B[g] = {
      scopeName: C,
      themeMatches: hC(Q, C, A.slice(0, g))
    };
  }
  return B;
}
...
function UA(Q, A, B, g = {
  ...
  e === !0 ? C = NC(C) : e === "never" && (C = rC(C)), i && (C = LC(C));
  const t = {
    ...g,
    get source() {
      return E;
...
let kC = 3;
function lC(Q, A = 3) {
  A > kC || console.trace(`[SHIKI DEPRECATE]: ${Q}`);
}
...
function YC(Q) {
  ...
  const B = (Q.langs || []).flat(1), g = (Q.themes || []).flat(1).map(iB), E = new UC(Q.engine, B), C = new KC(E, g, B, Q.langAlias);
  let I;
  function w(a) {
    G();
...
async function PC(Q) {
  Q.engine || lC("`engine` option is required. Use `createOnigurumaEngine` or `createJavaScriptRegexEngine` to create an engine.");
  const [
    A,
    B,
    g
...
async function pC(Q) {
  ...
  function w(t) {
    ...
    for (let G = 1; G <= 4; G *= 2) {
      ...
      const a = Math.min(N, SC(Math.max(t, h), 65536));
      if (I(a))
        return !0;
    }
    return !1;
...
var fC = Object.defineProperty, dC = (Q, A, B) => A in Q ? fC(Q, A, { enumerable: !0, configurable: !0, writable: !0, value: B }) : Q[A] = B, l = (Q, A, B) => dC(Q, typeof A != "symbol" ? A + "" : A, B);
let Y = null;
function OC(Q) {
...
class xC {
  constructor(A) {
    ...
    for (let w = 0, D = A.length; w < D; w++)
      Y.ofree(B[w]);
    Y.ofree(C), Y.ofree(E), I === 0 && OC(Y), this._onigBinding = Y, this._ptr = I;
  }
  dispose() {
...
async function VC(Q) {
  return Q && await vC(Q), {
    createScanner(A) {
      return new xC(A.map((B) => typeof B == "string" ? B : B.source));
    },
    createString(A) {
      return new RQ(A);
    }
...
export {
  wI as getTransformers,
  BI as getTransformersModule,
  DI as highlighterTask,
  oI as readyCopyButton
};
