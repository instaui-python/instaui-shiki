var kQ = Object.defineProperty;
var lQ = (Q, A, B) => A in Q ? kQ(Q, A, { enumerable: !0, configurable: !0, writable: !0, value: B }) : Q[A] = B;
var c = (Q, A, B) => lQ(Q, typeof A != "symbol" ? A + "" : A, B);
import { shallowRef as _, shallowReadonly as KQ, toValue as V, getCurrentScope as UQ, onScopeDispose as YQ, watch as OB, computed as YA, getCurrentInstance as PQ, onMounted as uQ, unref as yQ, toRaw as JQ, normalizeClass as SQ } from "vue";
function xB(Q) {
  return UQ() ? (YQ(Q), !0) : !1;
}
const gB = typeof window < "u" && typeof document < "u";
typeof WorkerGlobalScope < "u" && globalThis instanceof WorkerGlobalScope;
const pQ = Object.prototype.toString, fQ = (Q) => pQ.call(Q) === "[object Object]";
function dQ(Q) {
  let A;
  function B() {
    return A || (A = Q()), A;
  }
  return B.reset = async () => {
    const g = A;
    A = void 0, g && await g;
  }, B;
}
function fA(Q) {
  return Array.isArray(Q) ? Q : [Q];
}
function OQ(Q, A, B = {}) {
  const {
    immediate: g = !0,
    immediateCallback: E = !1
  } = B, C = _(!1);
  let I;
  function w() {
    I && (clearTimeout(I), I = void 0);
  }
  function D() {
    C.value = !1, w();
  }
  function o(...s) {
    E && Q(), w(), C.value = !0, I = setTimeout(() => {
      C.value = !1, I = void 0, Q(...s);
    }, V(A));
  }
  return g && (C.value = !0, gB && o()), xB(D), {
    isPending: KQ(C),
    start: o,
    stop: D
  };
}
function xQ(Q, A, B) {
  return OB(
    Q,
    A,
    {
      ...B,
      immediate: !0
    }
  );
}
const mQ = gB ? window : void 0, mB = gB ? window.navigator : void 0;
function qQ(Q) {
  var A;
  const B = V(Q);
  return (A = B == null ? void 0 : B.$el) != null ? A : B;
}
function qB(...Q) {
  const A = [], B = () => {
    A.forEach((w) => w()), A.length = 0;
  }, g = (w, D, o, s) => (w.addEventListener(D, o, s), () => w.removeEventListener(D, o, s)), E = YA(() => {
    const w = fA(V(Q[0])).filter((D) => D != null);
    return w.every((D) => typeof D != "string") ? w : void 0;
  }), C = xQ(
    () => {
      var w, D;
      return [
        (D = (w = E.value) == null ? void 0 : w.map((o) => qQ(o))) != null ? D : [mQ].filter((o) => o != null),
        fA(V(E.value ? Q[1] : Q[0])),
        fA(yQ(E.value ? Q[2] : Q[1])),
        // @ts-expect-error - TypeScript gets the correct types, but somehow still complains
        V(E.value ? Q[3] : Q[2])
      ];
    },
    ([w, D, o, s]) => {
      if (B(), !(w != null && w.length) || !(D != null && D.length) || !(o != null && o.length))
        return;
      const e = fQ(s) ? { ...s } : s;
      A.push(
        ...w.flatMap(
          (i) => D.flatMap(
            (t) => o.map((n) => g(i, t, n, e))
          )
        )
      );
    },
    { flush: "post" }
  ), I = () => {
    C(), B();
  };
  return xB(B), I;
}
function bQ() {
  const Q = _(!1), A = PQ();
  return A && uQ(() => {
    Q.value = !0;
  }, A), Q;
}
function bB(Q) {
  const A = bQ();
  return YA(() => (A.value, !!Q()));
}
function aB(Q, A = {}) {
  const {
    controls: B = !1,
    navigator: g = mB
  } = A, E = bB(() => g && "permissions" in g), C = _(), I = typeof Q == "string" ? { name: Q } : Q, w = _(), D = () => {
    var s, e;
    w.value = (e = (s = C.value) == null ? void 0 : s.state) != null ? e : "prompt";
  };
  qB(C, "change", D, { passive: !0 });
  const o = dQ(async () => {
    if (E.value) {
      if (!C.value)
        try {
          C.value = await g.permissions.query(I);
        } catch {
          C.value = void 0;
        } finally {
          D();
        }
      if (B)
        return JQ(C.value);
    }
  });
  return o(), B ? {
    state: w,
    isSupported: E,
    query: o
  } : w;
}
function TQ(Q = {}) {
  const {
    navigator: A = mB,
    read: B = !1,
    source: g,
    copiedDuring: E = 1500,
    legacy: C = !1
  } = Q, I = bB(() => A && "clipboard" in A), w = aB("clipboard-read"), D = aB("clipboard-write"), o = YA(() => I.value || C), s = _(""), e = _(!1), i = OQ(() => e.value = !1, E, { immediate: !1 });
  async function t() {
    let a = !(I.value && h(w.value));
    if (!a)
      try {
        s.value = await A.clipboard.readText();
      } catch {
        a = !0;
      }
    a && (s.value = G());
  }
  o.value && B && qB(["copy", "cut"], t, { passive: !0 });
  async function n(a = V(g)) {
    if (o.value && a != null) {
      let F = !(I.value && h(D.value));
      if (!F)
        try {
          await A.clipboard.writeText(a);
        } catch {
          F = !0;
        }
      F && N(a), s.value = a, e.value = !0, i.start();
    }
  }
  function N(a) {
    const F = document.createElement("textarea");
    F.value = a ?? "", F.style.position = "absolute", F.style.opacity = "0", document.body.appendChild(F), F.select(), document.execCommand("copy"), F.remove();
  }
  function G() {
    var a, F, L;
    return (L = (F = (a = document == null ? void 0 : document.getSelection) == null ? void 0 : a.call(document)) == null ? void 0 : F.toString()) != null ? L : "";
  }
  function h(a) {
    return a === "granted" || a === "prompt";
  }
  return {
    isSupported: o,
    text: s,
    copied: e,
    copy: n
  };
}
let P = class extends Error {
  constructor(A) {
    super(A), this.name = "ShikiError";
  }
};
function WQ(Q) {
  return EB(Q);
}
function EB(Q) {
  return Array.isArray(Q) ? vQ(Q) : Q instanceof RegExp ? Q : typeof Q == "object" ? XQ(Q) : Q;
}
function vQ(Q) {
  let A = [];
  for (let B = 0, g = Q.length; B < g; B++)
    A[B] = EB(Q[B]);
  return A;
}
function XQ(Q) {
  let A = {};
  for (let B in Q)
    A[B] = EB(Q[B]);
  return A;
}
function TB(Q, ...A) {
  return A.forEach((B) => {
    for (let g in B)
      Q[g] = B[g];
  }), Q;
}
function WB(Q) {
  const A = ~Q.lastIndexOf("/") || ~Q.lastIndexOf("\\");
  return A === 0 ? Q : ~A === Q.length - 1 ? WB(Q.substring(0, Q.length - 1)) : Q.substr(~A + 1);
}
var dA = /\$(\d+)|\${(\d+):\/(downcase|upcase)}/g, tA = class {
  static hasCaptures(Q) {
    return Q === null ? !1 : (dA.lastIndex = 0, dA.test(Q));
  }
  static replaceCaptures(Q, A, B) {
    return Q.replace(dA, (g, E, C, I) => {
      let w = B[parseInt(E || C, 10)];
      if (w) {
        let D = A.substring(w.start, w.end);
        for (; D[0] === "."; )
          D = D.substring(1);
        switch (I) {
          case "downcase":
            return D.toLowerCase();
          case "upcase":
            return D.toUpperCase();
          default:
            return D;
        }
      } else
        return g;
    });
  }
};
function vB(Q, A) {
  return Q < A ? -1 : Q > A ? 1 : 0;
}
function XB(Q, A) {
  if (Q === null && A === null)
    return 0;
  if (!Q)
    return -1;
  if (!A)
    return 1;
  let B = Q.length, g = A.length;
  if (B === g) {
    for (let E = 0; E < B; E++) {
      let C = vB(Q[E], A[E]);
      if (C !== 0)
        return C;
    }
    return 0;
  }
  return B - g;
}
function MB(Q) {
  return !!(/^#[0-9a-f]{6}$/i.test(Q) || /^#[0-9a-f]{8}$/i.test(Q) || /^#[0-9a-f]{3}$/i.test(Q) || /^#[0-9a-f]{4}$/i.test(Q));
}
function ZB(Q) {
  return Q.replace(/[\-\\\{\}\*\+\?\|\^\$\.\,\[\]\(\)\#\s]/g, "\\$&");
}
var VB = class {
  constructor(Q) {
    c(this, "cache", /* @__PURE__ */ new Map());
    this.fn = Q;
  }
  get(Q) {
    if (this.cache.has(Q))
      return this.cache.get(Q);
    const A = this.fn(Q);
    return this.cache.set(Q, A), A;
  }
}, FA = class {
  constructor(Q, A, B) {
    c(this, "_cachedMatchRoot", new VB(
      (Q) => this._root.match(Q)
    ));
    this._colorMap = Q, this._defaults = A, this._root = B;
  }
  static createFromRawTheme(Q, A) {
    return this.createFromParsedTheme(zQ(Q), A);
  }
  static createFromParsedTheme(Q, A) {
    return _Q(Q, A);
  }
  getColorMap() {
    return this._colorMap.getColorMap();
  }
  getDefaults() {
    return this._defaults;
  }
  match(Q) {
    if (Q === null)
      return this._defaults;
    const A = Q.scopeName, g = this._cachedMatchRoot.get(A).find(
      (E) => ZQ(Q.parent, E.parentScopes)
    );
    return g ? new zB(
      g.fontStyle,
      g.foreground,
      g.background
    ) : null;
  }
}, OA = class MA {
  constructor(A, B) {
    this.parent = A, this.scopeName = B;
  }
  static push(A, B) {
    for (const g of B)
      A = new MA(A, g);
    return A;
  }
  static from(...A) {
    let B = null;
    for (let g = 0; g < A.length; g++)
      B = new MA(B, A[g]);
    return B;
  }
  push(A) {
    return new MA(this, A);
  }
  getSegments() {
    let A = this;
    const B = [];
    for (; A; )
      B.push(A.scopeName), A = A.parent;
    return B.reverse(), B;
  }
  toString() {
    return this.getSegments().join(" ");
  }
  extends(A) {
    return this === A ? !0 : this.parent === null ? !1 : this.parent.extends(A);
  }
  getExtensionIfDefined(A) {
    const B = [];
    let g = this;
    for (; g && g !== A; )
      B.push(g.scopeName), g = g.parent;
    return g === A ? B.reverse() : void 0;
  }
};
function ZQ(Q, A) {
  if (A.length === 0)
    return !0;
  for (let B = 0; B < A.length; B++) {
    let g = A[B], E = !1;
    if (g === ">") {
      if (B === A.length - 1)
        return !1;
      g = A[++B], E = !0;
    }
    for (; Q && !VQ(Q.scopeName, g); ) {
      if (E)
        return !1;
      Q = Q.parent;
    }
    if (!Q)
      return !1;
    Q = Q.parent;
  }
  return !0;
}
function VQ(Q, A) {
  return A === Q || Q.startsWith(A) && Q[A.length] === ".";
}
var zB = class {
  constructor(Q, A, B) {
    this.fontStyle = Q, this.foregroundId = A, this.backgroundId = B;
  }
};
function zQ(Q) {
  if (!Q)
    return [];
  if (!Q.settings || !Array.isArray(Q.settings))
    return [];
  let A = Q.settings, B = [], g = 0;
  for (let E = 0, C = A.length; E < C; E++) {
    let I = A[E];
    if (!I.settings)
      continue;
    let w;
    if (typeof I.scope == "string") {
      let e = I.scope;
      e = e.replace(/^[,]+/, ""), e = e.replace(/[,]+$/, ""), w = e.split(",");
    } else Array.isArray(I.scope) ? w = I.scope : w = [""];
    let D = -1;
    if (typeof I.settings.fontStyle == "string") {
      D = 0;
      let e = I.settings.fontStyle.split(" ");
      for (let i = 0, t = e.length; i < t; i++)
        switch (e[i]) {
          case "italic":
            D = D | 1;
            break;
          case "bold":
            D = D | 2;
            break;
          case "underline":
            D = D | 4;
            break;
          case "strikethrough":
            D = D | 8;
            break;
        }
    }
    let o = null;
    typeof I.settings.foreground == "string" && MB(I.settings.foreground) && (o = I.settings.foreground);
    let s = null;
    typeof I.settings.background == "string" && MB(I.settings.background) && (s = I.settings.background);
    for (let e = 0, i = w.length; e < i; e++) {
      let n = w[e].trim().split(" "), N = n[n.length - 1], G = null;
      n.length > 1 && (G = n.slice(0, n.length - 1), G.reverse()), B[g++] = new jQ(
        N,
        G,
        E,
        D,
        o,
        s
      );
    }
  }
  return B;
}
var jQ = class {
  constructor(Q, A, B, g, E, C) {
    this.scope = Q, this.parentScopes = A, this.index = B, this.fontStyle = g, this.foreground = E, this.background = C;
  }
}, y = /* @__PURE__ */ ((Q) => (Q[Q.NotSet = -1] = "NotSet", Q[Q.None = 0] = "None", Q[Q.Italic = 1] = "Italic", Q[Q.Bold = 2] = "Bold", Q[Q.Underline = 4] = "Underline", Q[Q.Strikethrough = 8] = "Strikethrough", Q))(y || {});
function _Q(Q, A) {
  Q.sort((D, o) => {
    let s = vB(D.scope, o.scope);
    return s !== 0 || (s = XB(D.parentScopes, o.parentScopes), s !== 0) ? s : D.index - o.index;
  });
  let B = 0, g = "#000000", E = "#ffffff";
  for (; Q.length >= 1 && Q[0].scope === ""; ) {
    let D = Q.shift();
    D.fontStyle !== -1 && (B = D.fontStyle), D.foreground !== null && (g = D.foreground), D.background !== null && (E = D.background);
  }
  let C = new $Q(A), I = new zB(B, C.getId(g), C.getId(E)), w = new Bg(new vA(0, null, -1, 0, 0), []);
  for (let D = 0, o = Q.length; D < o; D++) {
    let s = Q[D];
    w.insert(0, s.scope, s.parentScopes, s.fontStyle, C.getId(s.foreground), C.getId(s.background));
  }
  return new FA(C, I, w);
}
var $Q = class {
  constructor(Q) {
    c(this, "_isFrozen");
    c(this, "_lastColorId");
    c(this, "_id2color");
    c(this, "_color2id");
    if (this._lastColorId = 0, this._id2color = [], this._color2id = /* @__PURE__ */ Object.create(null), Array.isArray(Q)) {
      this._isFrozen = !0;
      for (let A = 0, B = Q.length; A < B; A++)
        this._color2id[Q[A]] = A, this._id2color[A] = Q[A];
    } else
      this._isFrozen = !1;
  }
  getId(Q) {
    if (Q === null)
      return 0;
    Q = Q.toUpperCase();
    let A = this._color2id[Q];
    if (A)
      return A;
    if (this._isFrozen)
      throw new Error(`Missing color in color map - ${Q}`);
    return A = ++this._lastColorId, this._color2id[Q] = A, this._id2color[A] = Q, A;
  }
  getColorMap() {
    return this._id2color.slice(0);
  }
}, Ag = Object.freeze([]), vA = class jB {
  constructor(A, B, g, E, C) {
    c(this, "scopeDepth");
    c(this, "parentScopes");
    c(this, "fontStyle");
    c(this, "foreground");
    c(this, "background");
    this.scopeDepth = A, this.parentScopes = B || Ag, this.fontStyle = g, this.foreground = E, this.background = C;
  }
  clone() {
    return new jB(this.scopeDepth, this.parentScopes, this.fontStyle, this.foreground, this.background);
  }
  static cloneArr(A) {
    let B = [];
    for (let g = 0, E = A.length; g < E; g++)
      B[g] = A[g].clone();
    return B;
  }
  acceptOverwrite(A, B, g, E) {
    this.scopeDepth > A ? console.log("how did this happen?") : this.scopeDepth = A, B !== -1 && (this.fontStyle = B), g !== 0 && (this.foreground = g), E !== 0 && (this.background = E);
  }
}, Bg = class XA {
  constructor(A, B = [], g = {}) {
    c(this, "_rulesWithParentScopes");
    this._mainRule = A, this._children = g, this._rulesWithParentScopes = B;
  }
  static _cmpBySpecificity(A, B) {
    if (A.scopeDepth !== B.scopeDepth)
      return B.scopeDepth - A.scopeDepth;
    let g = 0, E = 0;
    for (; A.parentScopes[g] === ">" && g++, B.parentScopes[E] === ">" && E++, !(g >= A.parentScopes.length || E >= B.parentScopes.length); ) {
      const C = B.parentScopes[E].length - A.parentScopes[g].length;
      if (C !== 0)
        return C;
      g++, E++;
    }
    return B.parentScopes.length - A.parentScopes.length;
  }
  match(A) {
    if (A !== "") {
      let g = A.indexOf("."), E, C;
      if (g === -1 ? (E = A, C = "") : (E = A.substring(0, g), C = A.substring(g + 1)), this._children.hasOwnProperty(E))
        return this._children[E].match(C);
    }
    const B = this._rulesWithParentScopes.concat(this._mainRule);
    return B.sort(XA._cmpBySpecificity), B;
  }
  insert(A, B, g, E, C, I) {
    if (B === "") {
      this._doInsertHere(A, g, E, C, I);
      return;
    }
    let w = B.indexOf("."), D, o;
    w === -1 ? (D = B, o = "") : (D = B.substring(0, w), o = B.substring(w + 1));
    let s;
    this._children.hasOwnProperty(D) ? s = this._children[D] : (s = new XA(this._mainRule.clone(), vA.cloneArr(this._rulesWithParentScopes)), this._children[D] = s), s.insert(A + 1, o, g, E, C, I);
  }
  _doInsertHere(A, B, g, E, C) {
    if (B === null) {
      this._mainRule.acceptOverwrite(A, g, E, C);
      return;
    }
    for (let I = 0, w = this._rulesWithParentScopes.length; I < w; I++) {
      let D = this._rulesWithParentScopes[I];
      if (XB(D.parentScopes, B) === 0) {
        D.acceptOverwrite(A, g, E, C);
        return;
      }
    }
    g === -1 && (g = this._mainRule.fontStyle), E === 0 && (E = this._mainRule.foreground), C === 0 && (C = this._mainRule.background), this._rulesWithParentScopes.push(new vA(A, B, g, E, C));
  }
}, $ = class f {
  static toBinaryStr(A) {
    return A.toString(2).padStart(32, "0");
  }
  static print(A) {
    const B = f.getLanguageId(A), g = f.getTokenType(A), E = f.getFontStyle(A), C = f.getForeground(A), I = f.getBackground(A);
    console.log({
      languageId: B,
      tokenType: g,
      fontStyle: E,
      foreground: C,
      background: I
    });
  }
  static getLanguageId(A) {
    return (A & 255) >>> 0;
  }
  static getTokenType(A) {
    return (A & 768) >>> 8;
  }
  static containsBalancedBrackets(A) {
    return (A & 1024) !== 0;
  }
  static getFontStyle(A) {
    return (A & 30720) >>> 11;
  }
  static getForeground(A) {
    return (A & 16744448) >>> 15;
  }
  static getBackground(A) {
    return (A & 4278190080) >>> 24;
  }
  /**
   * Updates the fields in `metadata`.
   * A value of `0`, `NotSet` or `null` indicates that the corresponding field should be left as is.
   */
  static set(A, B, g, E, C, I, w) {
    let D = f.getLanguageId(A), o = f.getTokenType(A), s = f.containsBalancedBrackets(A) ? 1 : 0, e = f.getFontStyle(A), i = f.getForeground(A), t = f.getBackground(A);
    return B !== 0 && (D = B), g !== 8 && (o = g), E !== null && (s = E ? 1 : 0), C !== -1 && (e = C), I !== 0 && (i = I), w !== 0 && (t = w), (D << 0 | o << 8 | s << 10 | e << 11 | i << 15 | t << 24) >>> 0;
  }
};
function GA(Q, A) {
  const B = [], g = Qg(Q);
  let E = g.next();
  for (; E !== null; ) {
    let D = 0;
    if (E.length === 2 && E.charAt(1) === ":") {
      switch (E.charAt(0)) {
        case "R":
          D = 1;
          break;
        case "L":
          D = -1;
          break;
        default:
          console.log(`Unknown priority ${E} in scope selector`);
      }
      E = g.next();
    }
    let o = I();
    if (B.push({ matcher: o, priority: D }), E !== ",")
      break;
    E = g.next();
  }
  return B;
  function C() {
    if (E === "-") {
      E = g.next();
      const D = C();
      return (o) => !!D && !D(o);
    }
    if (E === "(") {
      E = g.next();
      const D = w();
      return E === ")" && (E = g.next()), D;
    }
    if (hB(E)) {
      const D = [];
      do
        D.push(E), E = g.next();
      while (hB(E));
      return (o) => A(D, o);
    }
    return null;
  }
  function I() {
    const D = [];
    let o = C();
    for (; o; )
      D.push(o), o = C();
    return (s) => D.every((e) => e(s));
  }
  function w() {
    const D = [];
    let o = I();
    for (; o && (D.push(o), E === "|" || E === ","); ) {
      do
        E = g.next();
      while (E === "|" || E === ",");
      o = I();
    }
    return (s) => D.some((e) => e(s));
  }
}
function hB(Q) {
  return !!Q && !!Q.match(/[\w\.:]+/);
}
function Qg(Q) {
  let A = /([LR]:|[\w\.:][\w\.:\-]*|[\,\|\-\(\)])/g, B = A.exec(Q);
  return {
    next: () => {
      if (!B)
        return null;
      const g = B[0];
      return B = A.exec(Q), g;
    }
  };
}
function _B(Q) {
  typeof Q.dispose == "function" && Q.dispose();
}
var IA = class {
  constructor(Q) {
    this.scopeName = Q;
  }
  toKey() {
    return this.scopeName;
  }
}, gg = class {
  constructor(Q, A) {
    this.scopeName = Q, this.ruleName = A;
  }
  toKey() {
    return `${this.scopeName}#${this.ruleName}`;
  }
}, Eg = class {
  constructor() {
    c(this, "_references", []);
    c(this, "_seenReferenceKeys", /* @__PURE__ */ new Set());
    c(this, "visitedRule", /* @__PURE__ */ new Set());
  }
  get references() {
    return this._references;
  }
  add(Q) {
    const A = Q.toKey();
    this._seenReferenceKeys.has(A) || (this._seenReferenceKeys.add(A), this._references.push(Q));
  }
}, Cg = class {
  constructor(Q, A) {
    c(this, "seenFullScopeRequests", /* @__PURE__ */ new Set());
    c(this, "seenPartialScopeRequests", /* @__PURE__ */ new Set());
    c(this, "Q");
    this.repo = Q, this.initialScopeName = A, this.seenFullScopeRequests.add(this.initialScopeName), this.Q = [new IA(this.initialScopeName)];
  }
  processQueue() {
    const Q = this.Q;
    this.Q = [];
    const A = new Eg();
    for (const B of Q)
      Ig(B, this.initialScopeName, this.repo, A);
    for (const B of A.references)
      if (B instanceof IA) {
        if (this.seenFullScopeRequests.has(B.scopeName))
          continue;
        this.seenFullScopeRequests.add(B.scopeName), this.Q.push(B);
      } else {
        if (this.seenFullScopeRequests.has(B.scopeName) || this.seenPartialScopeRequests.has(B.toKey()))
          continue;
        this.seenPartialScopeRequests.add(B.toKey()), this.Q.push(B);
      }
  }
};
function Ig(Q, A, B, g) {
  const E = B.lookup(Q.scopeName);
  if (!E) {
    if (Q.scopeName === A)
      throw new Error(`No grammar provided for <${A}>`);
    return;
  }
  const C = B.lookup(A);
  Q instanceof IA ? hA({ baseGrammar: C, selfGrammar: E }, g) : ZA(
    Q.ruleName,
    { baseGrammar: C, selfGrammar: E, repository: E.repository },
    g
  );
  const I = B.injections(Q.scopeName);
  if (I)
    for (const w of I)
      g.add(new IA(w));
}
function ZA(Q, A, B) {
  if (A.repository && A.repository[Q]) {
    const g = A.repository[Q];
    NA([g], A, B);
  }
}
function hA(Q, A) {
  Q.selfGrammar.patterns && Array.isArray(Q.selfGrammar.patterns) && NA(
    Q.selfGrammar.patterns,
    { ...Q, repository: Q.selfGrammar.repository },
    A
  ), Q.selfGrammar.injections && NA(
    Object.values(Q.selfGrammar.injections),
    { ...Q, repository: Q.selfGrammar.repository },
    A
  );
}
function NA(Q, A, B) {
  for (const g of Q) {
    if (B.visitedRule.has(g))
      continue;
    B.visitedRule.add(g);
    const E = g.repository ? TB({}, A.repository, g.repository) : A.repository;
    Array.isArray(g.patterns) && NA(g.patterns, { ...A, repository: E }, B);
    const C = g.include;
    if (!C)
      continue;
    const I = $B(C);
    switch (I.kind) {
      case 0:
        hA({ ...A, selfGrammar: A.baseGrammar }, B);
        break;
      case 1:
        hA(A, B);
        break;
      case 2:
        ZA(I.ruleName, { ...A, repository: E }, B);
        break;
      case 3:
      case 4:
        const w = I.scopeName === A.selfGrammar.scopeName ? A.selfGrammar : I.scopeName === A.baseGrammar.scopeName ? A.baseGrammar : void 0;
        if (w) {
          const D = { baseGrammar: A.baseGrammar, selfGrammar: w, repository: E };
          I.kind === 4 ? ZA(I.ruleName, D, B) : hA(D, B);
        } else
          I.kind === 4 ? B.add(new gg(I.scopeName, I.ruleName)) : B.add(new IA(I.scopeName));
        break;
    }
  }
}
var Dg = class {
  constructor() {
    c(this, "kind", 0);
  }
}, wg = class {
  constructor() {
    c(this, "kind", 1);
  }
}, og = class {
  constructor(Q) {
    c(this, "kind", 2);
    this.ruleName = Q;
  }
}, sg = class {
  constructor(Q) {
    c(this, "kind", 3);
    this.scopeName = Q;
  }
}, eg = class {
  constructor(Q, A) {
    c(this, "kind", 4);
    this.scopeName = Q, this.ruleName = A;
  }
};
function $B(Q) {
  if (Q === "$base")
    return new Dg();
  if (Q === "$self")
    return new wg();
  const A = Q.indexOf("#");
  if (A === -1)
    return new sg(Q);
  if (A === 0)
    return new og(Q.substring(1));
  {
    const B = Q.substring(0, A), g = Q.substring(A + 1);
    return new eg(B, g);
  }
}
var ig = /\\(\d+)/, FB = /\\(\d+)/g, tg = -1, AQ = -2;
var sA = class {
  constructor(Q, A, B, g) {
    c(this, "$location");
    c(this, "id");
    c(this, "_nameIsCapturing");
    c(this, "_name");
    c(this, "_contentNameIsCapturing");
    c(this, "_contentName");
    this.$location = Q, this.id = A, this._name = B || null, this._nameIsCapturing = tA.hasCaptures(this._name), this._contentName = g || null, this._contentNameIsCapturing = tA.hasCaptures(this._contentName);
  }
  get debugName() {
    const Q = this.$location ? `${WB(this.$location.filename)}:${this.$location.line}` : "unknown";
    return `${this.constructor.name}#${this.id} @ ${Q}`;
  }
  getName(Q, A) {
    return !this._nameIsCapturing || this._name === null || Q === null || A === null ? this._name : tA.replaceCaptures(this._name, Q, A);
  }
  getContentName(Q, A) {
    return !this._contentNameIsCapturing || this._contentName === null ? this._contentName : tA.replaceCaptures(this._contentName, Q, A);
  }
}, cg = class extends sA {
  constructor(A, B, g, E, C) {
    super(A, B, g, E);
    c(this, "retokenizeCapturedWithRuleId");
    this.retokenizeCapturedWithRuleId = C;
  }
  dispose() {
  }
  collectPatterns(A, B) {
    throw new Error("Not supported!");
  }
  compile(A, B) {
    throw new Error("Not supported!");
  }
  compileAG(A, B, g, E) {
    throw new Error("Not supported!");
  }
}, ng = class extends sA {
  constructor(A, B, g, E, C) {
    super(A, B, g, null);
    c(this, "_match");
    c(this, "captures");
    c(this, "_cachedCompiledPatterns");
    this._match = new DA(E, this.id), this.captures = C, this._cachedCompiledPatterns = null;
  }
  dispose() {
    this._cachedCompiledPatterns && (this._cachedCompiledPatterns.dispose(), this._cachedCompiledPatterns = null);
  }
  get debugMatchRegExp() {
    return `${this._match.source}`;
  }
  collectPatterns(A, B) {
    B.push(this._match);
  }
  compile(A, B) {
    return this._getCachedCompiledPatterns(A).compile(A);
  }
  compileAG(A, B, g, E) {
    return this._getCachedCompiledPatterns(A).compileAG(A, g, E);
  }
  _getCachedCompiledPatterns(A) {
    return this._cachedCompiledPatterns || (this._cachedCompiledPatterns = new wA(), this.collectPatterns(A, this._cachedCompiledPatterns)), this._cachedCompiledPatterns;
  }
}, GB = class extends sA {
  constructor(A, B, g, E, C) {
    super(A, B, g, E);
    c(this, "hasMissingPatterns");
    c(this, "patterns");
    c(this, "_cachedCompiledPatterns");
    this.patterns = C.patterns, this.hasMissingPatterns = C.hasMissingPatterns, this._cachedCompiledPatterns = null;
  }
  dispose() {
    this._cachedCompiledPatterns && (this._cachedCompiledPatterns.dispose(), this._cachedCompiledPatterns = null);
  }
  collectPatterns(A, B) {
    for (const g of this.patterns)
      A.getRule(g).collectPatterns(A, B);
  }
  compile(A, B) {
    return this._getCachedCompiledPatterns(A).compile(A);
  }
  compileAG(A, B, g, E) {
    return this._getCachedCompiledPatterns(A).compileAG(A, g, E);
  }
  _getCachedCompiledPatterns(A) {
    return this._cachedCompiledPatterns || (this._cachedCompiledPatterns = new wA(), this.collectPatterns(A, this._cachedCompiledPatterns)), this._cachedCompiledPatterns;
  }
}, VA = class extends sA {
  constructor(A, B, g, E, C, I, w, D, o, s) {
    super(A, B, g, E);
    c(this, "_begin");
    c(this, "beginCaptures");
    c(this, "_end");
    c(this, "endHasBackReferences");
    c(this, "endCaptures");
    c(this, "applyEndPatternLast");
    c(this, "hasMissingPatterns");
    c(this, "patterns");
    c(this, "_cachedCompiledPatterns");
    this._begin = new DA(C, this.id), this.beginCaptures = I, this._end = new DA(w || "￿", -1), this.endHasBackReferences = this._end.hasBackReferences, this.endCaptures = D, this.applyEndPatternLast = o || !1, this.patterns = s.patterns, this.hasMissingPatterns = s.hasMissingPatterns, this._cachedCompiledPatterns = null;
  }
  dispose() {
    this._cachedCompiledPatterns && (this._cachedCompiledPatterns.dispose(), this._cachedCompiledPatterns = null);
  }
  get debugBeginRegExp() {
    return `${this._begin.source}`;
  }
  get debugEndRegExp() {
    return `${this._end.source}`;
  }
  getEndWithResolvedBackReferences(A, B) {
    return this._end.resolveBackReferences(A, B);
  }
  collectPatterns(A, B) {
    B.push(this._begin);
  }
  compile(A, B) {
    return this._getCachedCompiledPatterns(A, B).compile(A);
  }
  compileAG(A, B, g, E) {
    return this._getCachedCompiledPatterns(A, B).compileAG(A, g, E);
  }
  _getCachedCompiledPatterns(A, B) {
    if (!this._cachedCompiledPatterns) {
      this._cachedCompiledPatterns = new wA();
      for (const g of this.patterns)
        A.getRule(g).collectPatterns(A, this._cachedCompiledPatterns);
      this.applyEndPatternLast ? this._cachedCompiledPatterns.push(this._end.hasBackReferences ? this._end.clone() : this._end) : this._cachedCompiledPatterns.unshift(this._end.hasBackReferences ? this._end.clone() : this._end);
    }
    return this._end.hasBackReferences && (this.applyEndPatternLast ? this._cachedCompiledPatterns.setSource(this._cachedCompiledPatterns.length() - 1, B) : this._cachedCompiledPatterns.setSource(0, B)), this._cachedCompiledPatterns;
  }
}, rA = class extends sA {
  constructor(A, B, g, E, C, I, w, D, o) {
    super(A, B, g, E);
    c(this, "_begin");
    c(this, "beginCaptures");
    c(this, "whileCaptures");
    c(this, "_while");
    c(this, "whileHasBackReferences");
    c(this, "hasMissingPatterns");
    c(this, "patterns");
    c(this, "_cachedCompiledPatterns");
    c(this, "_cachedCompiledWhilePatterns");
    this._begin = new DA(C, this.id), this.beginCaptures = I, this.whileCaptures = D, this._while = new DA(w, AQ), this.whileHasBackReferences = this._while.hasBackReferences, this.patterns = o.patterns, this.hasMissingPatterns = o.hasMissingPatterns, this._cachedCompiledPatterns = null, this._cachedCompiledWhilePatterns = null;
  }
  dispose() {
    this._cachedCompiledPatterns && (this._cachedCompiledPatterns.dispose(), this._cachedCompiledPatterns = null), this._cachedCompiledWhilePatterns && (this._cachedCompiledWhilePatterns.dispose(), this._cachedCompiledWhilePatterns = null);
  }
  get debugBeginRegExp() {
    return `${this._begin.source}`;
  }
  get debugWhileRegExp() {
    return `${this._while.source}`;
  }
  getWhileWithResolvedBackReferences(A, B) {
    return this._while.resolveBackReferences(A, B);
  }
  collectPatterns(A, B) {
    B.push(this._begin);
  }
  compile(A, B) {
    return this._getCachedCompiledPatterns(A).compile(A);
  }
  compileAG(A, B, g, E) {
    return this._getCachedCompiledPatterns(A).compileAG(A, g, E);
  }
  _getCachedCompiledPatterns(A) {
    if (!this._cachedCompiledPatterns) {
      this._cachedCompiledPatterns = new wA();
      for (const B of this.patterns)
        A.getRule(B).collectPatterns(A, this._cachedCompiledPatterns);
    }
    return this._cachedCompiledPatterns;
  }
  compileWhile(A, B) {
    return this._getCachedCompiledWhilePatterns(A, B).compile(A);
  }
  compileWhileAG(A, B, g, E) {
    return this._getCachedCompiledWhilePatterns(A, B).compileAG(A, g, E);
  }
  _getCachedCompiledWhilePatterns(A, B) {
    return this._cachedCompiledWhilePatterns || (this._cachedCompiledWhilePatterns = new wA(), this._cachedCompiledWhilePatterns.push(this._while.hasBackReferences ? this._while.clone() : this._while)), this._while.hasBackReferences && this._cachedCompiledWhilePatterns.setSource(0, B || "￿"), this._cachedCompiledWhilePatterns;
  }
}, BQ = class u {
  static createCaptureRule(A, B, g, E, C) {
    return A.registerRule((I) => new cg(B, I, g, E, C));
  }
  static getCompiledRuleId(A, B, g) {
    return A.id || B.registerRule((E) => {
      if (A.id = E, A.match)
        return new ng(
          A.$vscodeTextmateLocation,
          A.id,
          A.name,
          A.match,
          u._compileCaptures(A.captures, B, g)
        );
      if (typeof A.begin > "u") {
        A.repository && (g = TB({}, g, A.repository));
        let C = A.patterns;
        return typeof C > "u" && A.include && (C = [{ include: A.include }]), new GB(
          A.$vscodeTextmateLocation,
          A.id,
          A.name,
          A.contentName,
          u._compilePatterns(C, B, g)
        );
      }
      return A.while ? new rA(
        A.$vscodeTextmateLocation,
        A.id,
        A.name,
        A.contentName,
        A.begin,
        u._compileCaptures(A.beginCaptures || A.captures, B, g),
        A.while,
        u._compileCaptures(A.whileCaptures || A.captures, B, g),
        u._compilePatterns(A.patterns, B, g)
      ) : new VA(
        A.$vscodeTextmateLocation,
        A.id,
        A.name,
        A.contentName,
        A.begin,
        u._compileCaptures(A.beginCaptures || A.captures, B, g),
        A.end,
        u._compileCaptures(A.endCaptures || A.captures, B, g),
        A.applyEndPatternLast,
        u._compilePatterns(A.patterns, B, g)
      );
    }), A.id;
  }
  static _compileCaptures(A, B, g) {
    let E = [];
    if (A) {
      let C = 0;
      for (const I in A) {
        if (I === "$vscodeTextmateLocation")
          continue;
        const w = parseInt(I, 10);
        w > C && (C = w);
      }
      for (let I = 0; I <= C; I++)
        E[I] = null;
      for (const I in A) {
        if (I === "$vscodeTextmateLocation")
          continue;
        const w = parseInt(I, 10);
        let D = 0;
        A[I].patterns && (D = u.getCompiledRuleId(A[I], B, g)), E[w] = u.createCaptureRule(B, A[I].$vscodeTextmateLocation, A[I].name, A[I].contentName, D);
      }
    }
    return E;
  }
  static _compilePatterns(A, B, g) {
    let E = [];
    if (A)
      for (let C = 0, I = A.length; C < I; C++) {
        const w = A[C];
        let D = -1;
        if (w.include) {
          const o = $B(w.include);
          switch (o.kind) {
            case 0:
            case 1:
              D = u.getCompiledRuleId(g[w.include], B, g);
              break;
            case 2:
              let s = g[o.ruleName];
              s && (D = u.getCompiledRuleId(s, B, g));
              break;
            case 3:
            case 4:
              const e = o.scopeName, i = o.kind === 4 ? o.ruleName : null, t = B.getExternalGrammar(e, g);
              if (t)
                if (i) {
                  let n = t.repository[i];
                  n && (D = u.getCompiledRuleId(n, B, t.repository));
                } else
                  D = u.getCompiledRuleId(t.repository.$self, B, t.repository);
              break;
          }
        } else
          D = u.getCompiledRuleId(w, B, g);
        if (D !== -1) {
          const o = B.getRule(D);
          let s = !1;
          if ((o instanceof GB || o instanceof VA || o instanceof rA) && o.hasMissingPatterns && o.patterns.length === 0 && (s = !0), s)
            continue;
          E.push(D);
        }
      }
    return {
      patterns: E,
      hasMissingPatterns: (A ? A.length : 0) !== E.length
    };
  }
}, DA = class QQ {
  constructor(A, B) {
    c(this, "source");
    c(this, "ruleId");
    c(this, "hasAnchor");
    c(this, "hasBackReferences");
    c(this, "_anchorCache");
    if (A && typeof A == "string") {
      const g = A.length;
      let E = 0, C = [], I = !1;
      for (let w = 0; w < g; w++)
        if (A.charAt(w) === "\\" && w + 1 < g) {
          const o = A.charAt(w + 1);
          o === "z" ? (C.push(A.substring(E, w)), C.push("$(?!\\n)(?<!\\n)"), E = w + 2) : (o === "A" || o === "G") && (I = !0), w++;
        }
      this.hasAnchor = I, E === 0 ? this.source = A : (C.push(A.substring(E, g)), this.source = C.join(""));
    } else
      this.hasAnchor = !1, this.source = A;
    this.hasAnchor ? this._anchorCache = this._buildAnchorCache() : this._anchorCache = null, this.ruleId = B, typeof this.source == "string" ? this.hasBackReferences = ig.test(this.source) : this.hasBackReferences = !1;
  }
  clone() {
    return new QQ(this.source, this.ruleId);
  }
  setSource(A) {
    this.source !== A && (this.source = A, this.hasAnchor && (this._anchorCache = this._buildAnchorCache()));
  }
  resolveBackReferences(A, B) {
    if (typeof this.source != "string")
      throw new Error("This method should only be called if the source is a string");
    let g = B.map((E) => A.substring(E.start, E.end));
    return FB.lastIndex = 0, this.source.replace(FB, (E, C) => ZB(g[parseInt(C, 10)] || ""));
  }
  _buildAnchorCache() {
    if (typeof this.source != "string")
      throw new Error("This method should only be called if the source is a string");
    let A = [], B = [], g = [], E = [], C, I, w, D;
    for (C = 0, I = this.source.length; C < I; C++)
      w = this.source.charAt(C), A[C] = w, B[C] = w, g[C] = w, E[C] = w, w === "\\" && C + 1 < I && (D = this.source.charAt(C + 1), D === "A" ? (A[C + 1] = "￿", B[C + 1] = "￿", g[C + 1] = "A", E[C + 1] = "A") : D === "G" ? (A[C + 1] = "￿", B[C + 1] = "G", g[C + 1] = "￿", E[C + 1] = "G") : (A[C + 1] = D, B[C + 1] = D, g[C + 1] = D, E[C + 1] = D), C++);
    return {
      A0_G0: A.join(""),
      A0_G1: B.join(""),
      A1_G0: g.join(""),
      A1_G1: E.join("")
    };
  }
  resolveAnchors(A, B) {
    return !this.hasAnchor || !this._anchorCache || typeof this.source != "string" ? this.source : A ? B ? this._anchorCache.A1_G1 : this._anchorCache.A1_G0 : B ? this._anchorCache.A0_G1 : this._anchorCache.A0_G0;
  }
}, wA = class {
  constructor() {
    c(this, "_items");
    c(this, "_hasAnchors");
    c(this, "_cached");
    c(this, "_anchorCache");
    this._items = [], this._hasAnchors = !1, this._cached = null, this._anchorCache = {
      A0_G0: null,
      A0_G1: null,
      A1_G0: null,
      A1_G1: null
    };
  }
  dispose() {
    this._disposeCaches();
  }
  _disposeCaches() {
    this._cached && (this._cached.dispose(), this._cached = null), this._anchorCache.A0_G0 && (this._anchorCache.A0_G0.dispose(), this._anchorCache.A0_G0 = null), this._anchorCache.A0_G1 && (this._anchorCache.A0_G1.dispose(), this._anchorCache.A0_G1 = null), this._anchorCache.A1_G0 && (this._anchorCache.A1_G0.dispose(), this._anchorCache.A1_G0 = null), this._anchorCache.A1_G1 && (this._anchorCache.A1_G1.dispose(), this._anchorCache.A1_G1 = null);
  }
  push(Q) {
    this._items.push(Q), this._hasAnchors = this._hasAnchors || Q.hasAnchor;
  }
  unshift(Q) {
    this._items.unshift(Q), this._hasAnchors = this._hasAnchors || Q.hasAnchor;
  }
  length() {
    return this._items.length;
  }
  setSource(Q, A) {
    this._items[Q].source !== A && (this._disposeCaches(), this._items[Q].setSource(A));
  }
  compile(Q) {
    if (!this._cached) {
      let A = this._items.map((B) => B.source);
      this._cached = new NB(Q, A, this._items.map((B) => B.ruleId));
    }
    return this._cached;
  }
  compileAG(Q, A, B) {
    return this._hasAnchors ? A ? B ? (this._anchorCache.A1_G1 || (this._anchorCache.A1_G1 = this._resolveAnchors(Q, A, B)), this._anchorCache.A1_G1) : (this._anchorCache.A1_G0 || (this._anchorCache.A1_G0 = this._resolveAnchors(Q, A, B)), this._anchorCache.A1_G0) : B ? (this._anchorCache.A0_G1 || (this._anchorCache.A0_G1 = this._resolveAnchors(Q, A, B)), this._anchorCache.A0_G1) : (this._anchorCache.A0_G0 || (this._anchorCache.A0_G0 = this._resolveAnchors(Q, A, B)), this._anchorCache.A0_G0) : this.compile(Q);
  }
  _resolveAnchors(Q, A, B) {
    let g = this._items.map((E) => E.resolveAnchors(A, B));
    return new NB(Q, g, this._items.map((E) => E.ruleId));
  }
}, NB = class {
  constructor(Q, A, B) {
    c(this, "scanner");
    this.regExps = A, this.rules = B, this.scanner = Q.createOnigScanner(A);
  }
  dispose() {
    typeof this.scanner.dispose == "function" && this.scanner.dispose();
  }
  toString() {
    const Q = [];
    for (let A = 0, B = this.rules.length; A < B; A++)
      Q.push("   - " + this.rules[A] + ": " + this.regExps[A]);
    return Q.join(`
`);
  }
  findNextMatchSync(Q, A, B) {
    const g = this.scanner.findNextMatchSync(Q, A, B);
    return g ? {
      ruleId: this.rules[g.index],
      captureIndices: g.captureIndices
    } : null;
  }
}, xA = class {
  constructor(Q, A) {
    this.languageId = Q, this.tokenType = A;
  }
}, b, ag = (b = class {
  constructor(A, B) {
    c(this, "_defaultAttributes");
    c(this, "_embeddedLanguagesMatcher");
    c(this, "_getBasicScopeAttributes", new VB((A) => {
      const B = this._scopeToLanguage(A), g = this._toStandardTokenType(A);
      return new xA(B, g);
    }));
    this._defaultAttributes = new xA(
      A,
      8
      /* NotSet */
    ), this._embeddedLanguagesMatcher = new Mg(Object.entries(B || {}));
  }
  getDefaultAttributes() {
    return this._defaultAttributes;
  }
  getBasicScopeAttributes(A) {
    return A === null ? b._NULL_SCOPE_METADATA : this._getBasicScopeAttributes.get(A);
  }
  /**
   * Given a produced TM scope, return the language that token describes or null if unknown.
   * e.g. source.html => html, source.css.embedded.html => css, punctuation.definition.tag.html => null
   */
  _scopeToLanguage(A) {
    return this._embeddedLanguagesMatcher.match(A) || 0;
  }
  _toStandardTokenType(A) {
    const B = A.match(b.STANDARD_TOKEN_TYPE_REGEXP);
    if (!B)
      return 8;
    switch (B[1]) {
      case "comment":
        return 1;
      case "string":
        return 2;
      case "regex":
        return 3;
      case "meta.embedded":
        return 0;
    }
    throw new Error("Unexpected match for standard token type!");
  }
}, c(b, "_NULL_SCOPE_METADATA", new xA(0, 0)), c(b, "STANDARD_TOKEN_TYPE_REGEXP", /\b(comment|string|regex|meta\.embedded)\b/), b), Mg = class {
  constructor(Q) {
    c(this, "values");
    c(this, "scopesRegExp");
    if (Q.length === 0)
      this.values = null, this.scopesRegExp = null;
    else {
      this.values = new Map(Q);
      const A = Q.map(
        ([B, g]) => ZB(B)
      );
      A.sort(), A.reverse(), this.scopesRegExp = new RegExp(
        `^((${A.join(")|(")}))($|\\.)`,
        ""
      );
    }
  }
  match(Q) {
    if (!this.scopesRegExp)
      return;
    const A = Q.match(this.scopesRegExp);
    if (A)
      return this.values.get(A[1]);
  }
};
typeof process < "u" && process.env.VSCODE_TEXTMATE_DEBUG;
var rB = class {
  constructor(Q, A) {
    this.stack = Q, this.stoppedEarly = A;
  }
};
function gQ(Q, A, B, g, E, C, I, w) {
  const D = A.content.length;
  let o = !1, s = -1;
  if (I) {
    const t = hg(
      Q,
      A,
      B,
      g,
      E,
      C
    );
    E = t.stack, g = t.linePos, B = t.isFirstLine, s = t.anchorPosition;
  }
  const e = Date.now();
  for (; !o; ) {
    if (w !== 0 && Date.now() - e > w)
      return new rB(E, !0);
    i();
  }
  return new rB(E, !1);
  function i() {
    const t = Fg(
      Q,
      A,
      B,
      g,
      E,
      s
    );
    if (!t) {
      C.produce(E, D), o = !0;
      return;
    }
    const n = t.captureIndices, N = t.matchedRuleId, G = n && n.length > 0 ? n[0].end > g : !1;
    if (N === tg) {
      const h = E.getRule(Q);
      C.produce(E, n[0].start), E = E.withContentNameScopesList(E.nameScopesList), EA(
        Q,
        A,
        B,
        E,
        C,
        h.endCaptures,
        n
      ), C.produce(E, n[0].end);
      const a = E;
      if (E = E.parent, s = a.getAnchorPos(), !G && a.getEnterPos() === g) {
        E = a, C.produce(E, D), o = !0;
        return;
      }
    } else {
      const h = Q.getRule(N);
      C.produce(E, n[0].start);
      const a = E, F = h.getName(A.content, n), L = E.contentNameScopesList.pushAttributed(
        F,
        Q
      );
      if (E = E.push(
        N,
        g,
        s,
        n[0].end === D,
        null,
        L,
        L
      ), h instanceof VA) {
        const R = h;
        EA(
          Q,
          A,
          B,
          E,
          C,
          R.beginCaptures,
          n
        ), C.produce(E, n[0].end), s = n[0].end;
        const J = R.getContentName(
          A.content,
          n
        ), U = L.pushAttributed(
          J,
          Q
        );
        if (E = E.withContentNameScopesList(U), R.endHasBackReferences && (E = E.withEndRule(
          R.getEndWithResolvedBackReferences(
            A.content,
            n
          )
        )), !G && a.hasSameRuleAs(E)) {
          E = E.pop(), C.produce(E, D), o = !0;
          return;
        }
      } else if (h instanceof rA) {
        const R = h;
        EA(
          Q,
          A,
          B,
          E,
          C,
          R.beginCaptures,
          n
        ), C.produce(E, n[0].end), s = n[0].end;
        const J = R.getContentName(
          A.content,
          n
        ), U = L.pushAttributed(
          J,
          Q
        );
        if (E = E.withContentNameScopesList(U), R.whileHasBackReferences && (E = E.withEndRule(
          R.getWhileWithResolvedBackReferences(
            A.content,
            n
          )
        )), !G && a.hasSameRuleAs(E)) {
          E = E.pop(), C.produce(E, D), o = !0;
          return;
        }
      } else if (EA(
        Q,
        A,
        B,
        E,
        C,
        h.captures,
        n
      ), C.produce(E, n[0].end), E = E.pop(), !G) {
        E = E.safePop(), C.produce(E, D), o = !0;
        return;
      }
    }
    n[0].end > g && (g = n[0].end, B = !1);
  }
}
function hg(Q, A, B, g, E, C) {
  let I = E.beginRuleCapturedEOL ? 0 : -1;
  const w = [];
  for (let D = E; D; D = D.pop()) {
    const o = D.getRule(Q);
    o instanceof rA && w.push({
      rule: o,
      stack: D
    });
  }
  for (let D = w.pop(); D; D = w.pop()) {
    const { ruleScanner: o, findOptions: s } = rg(D.rule, Q, D.stack.endRule, B, g === I), e = o.findNextMatchSync(A, g, s);
    if (e) {
      if (e.ruleId !== AQ) {
        E = D.stack.pop();
        break;
      }
      e.captureIndices && e.captureIndices.length && (C.produce(D.stack, e.captureIndices[0].start), EA(Q, A, B, D.stack, C, D.rule.whileCaptures, e.captureIndices), C.produce(D.stack, e.captureIndices[0].end), I = e.captureIndices[0].end, e.captureIndices[0].end > g && (g = e.captureIndices[0].end, B = !1));
    } else {
      E = D.stack.pop();
      break;
    }
  }
  return { stack: E, linePos: g, anchorPosition: I, isFirstLine: B };
}
function Fg(Q, A, B, g, E, C) {
  const I = Gg(Q, A, B, g, E, C), w = Q.getInjections();
  if (w.length === 0)
    return I;
  const D = Ng(w, Q, A, B, g, E, C);
  if (!D)
    return I;
  if (!I)
    return D;
  const o = I.captureIndices[0].start, s = D.captureIndices[0].start;
  return s < o || D.priorityMatch && s === o ? D : I;
}
function Gg(Q, A, B, g, E, C) {
  const I = E.getRule(Q), { ruleScanner: w, findOptions: D } = EQ(I, Q, E.endRule, B, g === C), o = w.findNextMatchSync(A, g, D);
  return o ? {
    captureIndices: o.captureIndices,
    matchedRuleId: o.ruleId
  } : null;
}
function Ng(Q, A, B, g, E, C, I) {
  let w = Number.MAX_VALUE, D = null, o, s = 0;
  const e = C.contentNameScopesList.getScopeNames();
  for (let i = 0, t = Q.length; i < t; i++) {
    const n = Q[i];
    if (!n.matcher(e))
      continue;
    const N = A.getRule(n.ruleId), { ruleScanner: G, findOptions: h } = EQ(N, A, null, g, E === I), a = G.findNextMatchSync(B, E, h);
    if (!a)
      continue;
    const F = a.captureIndices[0].start;
    if (!(F >= w) && (w = F, D = a.captureIndices, o = a.ruleId, s = n.priority, w === E))
      break;
  }
  return D ? {
    priorityMatch: s === -1,
    captureIndices: D,
    matchedRuleId: o
  } : null;
}
function EQ(Q, A, B, g, E) {
  return {
    ruleScanner: Q.compileAG(A, B, g, E),
    findOptions: 0
    /* None */
  };
}
function rg(Q, A, B, g, E) {
  return {
    ruleScanner: Q.compileWhileAG(A, B, g, E),
    findOptions: 0
    /* None */
  };
}
function EA(Q, A, B, g, E, C, I) {
  if (C.length === 0)
    return;
  const w = A.content, D = Math.min(C.length, I.length), o = [], s = I[0].end;
  for (let e = 0; e < D; e++) {
    const i = C[e];
    if (i === null)
      continue;
    const t = I[e];
    if (t.length === 0)
      continue;
    if (t.start > s)
      break;
    for (; o.length > 0 && o[o.length - 1].endPos <= t.start; )
      E.produceFromScopes(o[o.length - 1].scopes, o[o.length - 1].endPos), o.pop();
    if (o.length > 0 ? E.produceFromScopes(o[o.length - 1].scopes, t.start) : E.produce(g, t.start), i.retokenizeCapturedWithRuleId) {
      const N = i.getName(w, I), G = g.contentNameScopesList.pushAttributed(N, Q), h = i.getContentName(w, I), a = G.pushAttributed(h, Q), F = g.push(i.retokenizeCapturedWithRuleId, t.start, -1, !1, null, G, a), L = Q.createOnigString(w.substring(0, t.end));
      gQ(
        Q,
        L,
        B && t.start === 0,
        t.start,
        F,
        E,
        !1,
        /* no time limit */
        0
      ), _B(L);
      continue;
    }
    const n = i.getName(w, I);
    if (n !== null) {
      const G = (o.length > 0 ? o[o.length - 1].scopes : g.contentNameScopesList).pushAttributed(n, Q);
      o.push(new Lg(G, t.end));
    }
  }
  for (; o.length > 0; )
    E.produceFromScopes(o[o.length - 1].scopes, o[o.length - 1].endPos), o.pop();
}
var Lg = class {
  constructor(Q, A) {
    c(this, "scopes");
    c(this, "endPos");
    this.scopes = Q, this.endPos = A;
  }
};
function Hg(Q, A, B, g, E, C, I, w) {
  return new kg(
    Q,
    A,
    B,
    g,
    E,
    C,
    I,
    w
  );
}
function LB(Q, A, B, g, E) {
  const C = GA(A, LA), I = BQ.getCompiledRuleId(B, g, E.repository);
  for (const w of C)
    Q.push({
      debugSelector: A,
      matcher: w.matcher,
      ruleId: I,
      grammar: E,
      priority: w.priority
    });
}
function LA(Q, A) {
  if (A.length < Q.length)
    return !1;
  let B = 0;
  return Q.every((g) => {
    for (let E = B; E < A.length; E++)
      if (Rg(A[E], g))
        return B = E + 1, !0;
    return !1;
  });
}
function Rg(Q, A) {
  if (!Q)
    return !1;
  if (Q === A)
    return !0;
  const B = A.length;
  return Q.length > B && Q.substr(0, B) === A && Q[B] === ".";
}
var kg = class {
  constructor(Q, A, B, g, E, C, I, w) {
    c(this, "_rootId");
    c(this, "_lastRuleId");
    c(this, "_ruleId2desc");
    c(this, "_includedGrammars");
    c(this, "_grammarRepository");
    c(this, "_grammar");
    c(this, "_injections");
    c(this, "_basicScopeAttributesProvider");
    c(this, "_tokenTypeMatchers");
    if (this._rootScopeName = Q, this.balancedBracketSelectors = C, this._onigLib = w, this._basicScopeAttributesProvider = new ag(
      B,
      g
    ), this._rootId = -1, this._lastRuleId = 0, this._ruleId2desc = [null], this._includedGrammars = {}, this._grammarRepository = I, this._grammar = HB(A, null), this._injections = null, this._tokenTypeMatchers = [], E)
      for (const D of Object.keys(E)) {
        const o = GA(D, LA);
        for (const s of o)
          this._tokenTypeMatchers.push({
            matcher: s.matcher,
            type: E[D]
          });
      }
  }
  get themeProvider() {
    return this._grammarRepository;
  }
  dispose() {
    for (const Q of this._ruleId2desc)
      Q && Q.dispose();
  }
  createOnigScanner(Q) {
    return this._onigLib.createOnigScanner(Q);
  }
  createOnigString(Q) {
    return this._onigLib.createOnigString(Q);
  }
  getMetadataForScope(Q) {
    return this._basicScopeAttributesProvider.getBasicScopeAttributes(Q);
  }
  _collectInjections() {
    const Q = {
      lookup: (E) => E === this._rootScopeName ? this._grammar : this.getExternalGrammar(E),
      injections: (E) => this._grammarRepository.injections(E)
    }, A = [], B = this._rootScopeName, g = Q.lookup(B);
    if (g) {
      const E = g.injections;
      if (E)
        for (let I in E)
          LB(
            A,
            I,
            E[I],
            this,
            g
          );
      const C = this._grammarRepository.injections(B);
      C && C.forEach((I) => {
        const w = this.getExternalGrammar(I);
        if (w) {
          const D = w.injectionSelector;
          D && LB(
            A,
            D,
            w,
            this,
            w
          );
        }
      });
    }
    return A.sort((E, C) => E.priority - C.priority), A;
  }
  getInjections() {
    return this._injections === null && (this._injections = this._collectInjections()), this._injections;
  }
  registerRule(Q) {
    const A = ++this._lastRuleId, B = Q(A);
    return this._ruleId2desc[A] = B, B;
  }
  getRule(Q) {
    return this._ruleId2desc[Q];
  }
  getExternalGrammar(Q, A) {
    if (this._includedGrammars[Q])
      return this._includedGrammars[Q];
    if (this._grammarRepository) {
      const B = this._grammarRepository.lookup(Q);
      if (B)
        return this._includedGrammars[Q] = HB(
          B,
          A && A.$base
        ), this._includedGrammars[Q];
    }
  }
  tokenizeLine(Q, A, B = 0) {
    const g = this._tokenize(Q, A, !1, B);
    return {
      tokens: g.lineTokens.getResult(g.ruleStack, g.lineLength),
      ruleStack: g.ruleStack,
      stoppedEarly: g.stoppedEarly
    };
  }
  tokenizeLine2(Q, A, B = 0) {
    const g = this._tokenize(Q, A, !0, B);
    return {
      tokens: g.lineTokens.getBinaryResult(g.ruleStack, g.lineLength),
      ruleStack: g.ruleStack,
      stoppedEarly: g.stoppedEarly
    };
  }
  _tokenize(Q, A, B, g) {
    this._rootId === -1 && (this._rootId = BQ.getCompiledRuleId(
      this._grammar.repository.$self,
      this,
      this._grammar.repository
    ), this.getInjections());
    let E;
    if (!A || A === zA.NULL) {
      E = !0;
      const o = this._basicScopeAttributesProvider.getDefaultAttributes(), s = this.themeProvider.getDefaults(), e = $.set(
        0,
        o.languageId,
        o.tokenType,
        null,
        s.fontStyle,
        s.foregroundId,
        s.backgroundId
      ), i = this.getRule(this._rootId).getName(
        null,
        null
      );
      let t;
      i ? t = CA.createRootAndLookUpScopeName(
        i,
        e,
        this
      ) : t = CA.createRoot(
        "unknown",
        e
      ), A = new zA(
        null,
        this._rootId,
        -1,
        -1,
        !1,
        null,
        t,
        t
      );
    } else
      E = !1, A.reset();
    Q = Q + `
`;
    const C = this.createOnigString(Q), I = C.content.length, w = new Kg(
      B,
      Q,
      this._tokenTypeMatchers,
      this.balancedBracketSelectors
    ), D = gQ(
      this,
      C,
      E,
      0,
      A,
      w,
      !0,
      g
    );
    return _B(C), {
      lineLength: I,
      lineTokens: w,
      ruleStack: D.stack,
      stoppedEarly: D.stoppedEarly
    };
  }
};
function HB(Q, A) {
  return Q = WQ(Q), Q.repository = Q.repository || {}, Q.repository.$self = {
    $vscodeTextmateLocation: Q.$vscodeTextmateLocation,
    patterns: Q.patterns,
    name: Q.scopeName
  }, Q.repository.$base = A || Q.repository.$self, Q;
}
var CA = class O {
  /**
   * Invariant:
   * ```
   * if (parent && !scopePath.extends(parent.scopePath)) {
   * 	throw new Error();
   * }
   * ```
   */
  constructor(A, B, g) {
    this.parent = A, this.scopePath = B, this.tokenAttributes = g;
  }
  static fromExtension(A, B) {
    let g = A, E = (A == null ? void 0 : A.scopePath) ?? null;
    for (const C of B)
      E = OA.push(E, C.scopeNames), g = new O(g, E, C.encodedTokenAttributes);
    return g;
  }
  static createRoot(A, B) {
    return new O(null, new OA(null, A), B);
  }
  static createRootAndLookUpScopeName(A, B, g) {
    const E = g.getMetadataForScope(A), C = new OA(null, A), I = g.themeProvider.themeMatch(C), w = O.mergeAttributes(
      B,
      E,
      I
    );
    return new O(null, C, w);
  }
  get scopeName() {
    return this.scopePath.scopeName;
  }
  toString() {
    return this.getScopeNames().join(" ");
  }
  equals(A) {
    return O.equals(this, A);
  }
  static equals(A, B) {
    do {
      if (A === B || !A && !B)
        return !0;
      if (!A || !B || A.scopeName !== B.scopeName || A.tokenAttributes !== B.tokenAttributes)
        return !1;
      A = A.parent, B = B.parent;
    } while (!0);
  }
  static mergeAttributes(A, B, g) {
    let E = -1, C = 0, I = 0;
    return g !== null && (E = g.fontStyle, C = g.foregroundId, I = g.backgroundId), $.set(
      A,
      B.languageId,
      B.tokenType,
      null,
      E,
      C,
      I
    );
  }
  pushAttributed(A, B) {
    if (A === null)
      return this;
    if (A.indexOf(" ") === -1)
      return O._pushAttributed(this, A, B);
    const g = A.split(/ /g);
    let E = this;
    for (const C of g)
      E = O._pushAttributed(E, C, B);
    return E;
  }
  static _pushAttributed(A, B, g) {
    const E = g.getMetadataForScope(B), C = A.scopePath.push(B), I = g.themeProvider.themeMatch(C), w = O.mergeAttributes(
      A.tokenAttributes,
      E,
      I
    );
    return new O(A, C, w);
  }
  getScopeNames() {
    return this.scopePath.getSegments();
  }
  getExtensionIfDefined(A) {
    var E;
    const B = [];
    let g = this;
    for (; g && g !== A; )
      B.push({
        encodedTokenAttributes: g.tokenAttributes,
        scopeNames: g.scopePath.getExtensionIfDefined(((E = g.parent) == null ? void 0 : E.scopePath) ?? null)
      }), g = g.parent;
    return g === A ? B.reverse() : void 0;
  }
}, d, zA = (d = class {
  /**
   * Invariant:
   * ```
   * if (contentNameScopesList !== nameScopesList && contentNameScopesList?.parent !== nameScopesList) {
   * 	throw new Error();
   * }
   * if (this.parent && !nameScopesList.extends(this.parent.contentNameScopesList)) {
   * 	throw new Error();
   * }
   * ```
   */
  constructor(A, B, g, E, C, I, w, D) {
    c(this, "_stackElementBrand");
    /**
     * The position on the current line where this state was pushed.
     * This is relevant only while tokenizing a line, to detect endless loops.
     * Its value is meaningless across lines.
     */
    c(this, "_enterPos");
    /**
     * The captured anchor position when this stack element was pushed.
     * This is relevant only while tokenizing a line, to restore the anchor position when popping.
     * Its value is meaningless across lines.
     */
    c(this, "_anchorPos");
    /**
     * The depth of the stack.
     */
    c(this, "depth");
    this.parent = A, this.ruleId = B, this.beginRuleCapturedEOL = C, this.endRule = I, this.nameScopesList = w, this.contentNameScopesList = D, this.depth = this.parent ? this.parent.depth + 1 : 1, this._enterPos = g, this._anchorPos = E;
  }
  equals(A) {
    return A === null ? !1 : d._equals(this, A);
  }
  static _equals(A, B) {
    return A === B ? !0 : this._structuralEquals(A, B) ? CA.equals(A.contentNameScopesList, B.contentNameScopesList) : !1;
  }
  /**
   * A structural equals check. Does not take into account `scopes`.
   */
  static _structuralEquals(A, B) {
    do {
      if (A === B || !A && !B)
        return !0;
      if (!A || !B || A.depth !== B.depth || A.ruleId !== B.ruleId || A.endRule !== B.endRule)
        return !1;
      A = A.parent, B = B.parent;
    } while (!0);
  }
  clone() {
    return this;
  }
  static _reset(A) {
    for (; A; )
      A._enterPos = -1, A._anchorPos = -1, A = A.parent;
  }
  reset() {
    d._reset(this);
  }
  pop() {
    return this.parent;
  }
  safePop() {
    return this.parent ? this.parent : this;
  }
  push(A, B, g, E, C, I, w) {
    return new d(
      this,
      A,
      B,
      g,
      E,
      C,
      I,
      w
    );
  }
  getEnterPos() {
    return this._enterPos;
  }
  getAnchorPos() {
    return this._anchorPos;
  }
  getRule(A) {
    return A.getRule(this.ruleId);
  }
  toString() {
    const A = [];
    return this._writeString(A, 0), "[" + A.join(",") + "]";
  }
  _writeString(A, B) {
    var g, E;
    return this.parent && (B = this.parent._writeString(A, B)), A[B++] = `(${this.ruleId}, ${(g = this.nameScopesList) == null ? void 0 : g.toString()}, ${(E = this.contentNameScopesList) == null ? void 0 : E.toString()})`, B;
  }
  withContentNameScopesList(A) {
    return this.contentNameScopesList === A ? this : this.parent.push(
      this.ruleId,
      this._enterPos,
      this._anchorPos,
      this.beginRuleCapturedEOL,
      this.endRule,
      this.nameScopesList,
      A
    );
  }
  withEndRule(A) {
    return this.endRule === A ? this : new d(
      this.parent,
      this.ruleId,
      this._enterPos,
      this._anchorPos,
      this.beginRuleCapturedEOL,
      A,
      this.nameScopesList,
      this.contentNameScopesList
    );
  }
  // Used to warn of endless loops
  hasSameRuleAs(A) {
    let B = this;
    for (; B && B._enterPos === A._enterPos; ) {
      if (B.ruleId === A.ruleId)
        return !0;
      B = B.parent;
    }
    return !1;
  }
  toStateStackFrame() {
    var A, B, g;
    return {
      ruleId: this.ruleId,
      beginRuleCapturedEOL: this.beginRuleCapturedEOL,
      endRule: this.endRule,
      nameScopesList: ((B = this.nameScopesList) == null ? void 0 : B.getExtensionIfDefined(((A = this.parent) == null ? void 0 : A.nameScopesList) ?? null)) ?? [],
      contentNameScopesList: ((g = this.contentNameScopesList) == null ? void 0 : g.getExtensionIfDefined(this.nameScopesList)) ?? []
    };
  }
  static pushFrame(A, B) {
    const g = CA.fromExtension((A == null ? void 0 : A.nameScopesList) ?? null, B.nameScopesList);
    return new d(
      A,
      B.ruleId,
      B.enterPos ?? -1,
      B.anchorPos ?? -1,
      B.beginRuleCapturedEOL,
      B.endRule,
      g,
      CA.fromExtension(g, B.contentNameScopesList)
    );
  }
}, // TODO remove me
c(d, "NULL", new d(
  null,
  0,
  0,
  0,
  !1,
  null,
  null,
  null
)), d), lg = class {
  constructor(Q, A) {
    c(this, "balancedBracketScopes");
    c(this, "unbalancedBracketScopes");
    c(this, "allowAny", !1);
    this.balancedBracketScopes = Q.flatMap(
      (B) => B === "*" ? (this.allowAny = !0, []) : GA(B, LA).map((g) => g.matcher)
    ), this.unbalancedBracketScopes = A.flatMap(
      (B) => GA(B, LA).map((g) => g.matcher)
    );
  }
  get matchesAlways() {
    return this.allowAny && this.unbalancedBracketScopes.length === 0;
  }
  get matchesNever() {
    return this.balancedBracketScopes.length === 0 && !this.allowAny;
  }
  match(Q) {
    for (const A of this.unbalancedBracketScopes)
      if (A(Q))
        return !1;
    for (const A of this.balancedBracketScopes)
      if (A(Q))
        return !0;
    return this.allowAny;
  }
}, Kg = class {
  constructor(Q, A, B, g) {
    c(this, "_emitBinaryTokens");
    /**
     * defined only if `false`.
     */
    c(this, "_lineText");
    /**
     * used only if `_emitBinaryTokens` is false.
     */
    c(this, "_tokens");
    /**
     * used only if `_emitBinaryTokens` is true.
     */
    c(this, "_binaryTokens");
    c(this, "_lastTokenEndIndex");
    c(this, "_tokenTypeOverrides");
    this.balancedBracketSelectors = g, this._emitBinaryTokens = Q, this._tokenTypeOverrides = B, this._lineText = null, this._tokens = [], this._binaryTokens = [], this._lastTokenEndIndex = 0;
  }
  produce(Q, A) {
    this.produceFromScopes(Q.contentNameScopesList, A);
  }
  produceFromScopes(Q, A) {
    var g;
    if (this._lastTokenEndIndex >= A)
      return;
    if (this._emitBinaryTokens) {
      let E = (Q == null ? void 0 : Q.tokenAttributes) ?? 0, C = !1;
      if ((g = this.balancedBracketSelectors) != null && g.matchesAlways && (C = !0), this._tokenTypeOverrides.length > 0 || this.balancedBracketSelectors && !this.balancedBracketSelectors.matchesAlways && !this.balancedBracketSelectors.matchesNever) {
        const I = (Q == null ? void 0 : Q.getScopeNames()) ?? [];
        for (const w of this._tokenTypeOverrides)
          w.matcher(I) && (E = $.set(
            E,
            0,
            w.type,
            null,
            -1,
            0,
            0
          ));
        this.balancedBracketSelectors && (C = this.balancedBracketSelectors.match(I));
      }
      if (C && (E = $.set(
        E,
        0,
        8,
        C,
        -1,
        0,
        0
      )), this._binaryTokens.length > 0 && this._binaryTokens[this._binaryTokens.length - 1] === E) {
        this._lastTokenEndIndex = A;
        return;
      }
      this._binaryTokens.push(this._lastTokenEndIndex), this._binaryTokens.push(E), this._lastTokenEndIndex = A;
      return;
    }
    const B = (Q == null ? void 0 : Q.getScopeNames()) ?? [];
    this._tokens.push({
      startIndex: this._lastTokenEndIndex,
      endIndex: A,
      // value: lineText.substring(lastTokenEndIndex, endIndex),
      scopes: B
    }), this._lastTokenEndIndex = A;
  }
  getResult(Q, A) {
    return this._tokens.length > 0 && this._tokens[this._tokens.length - 1].startIndex === A - 1 && this._tokens.pop(), this._tokens.length === 0 && (this._lastTokenEndIndex = -1, this.produce(Q, A), this._tokens[this._tokens.length - 1].startIndex = 0), this._tokens;
  }
  getBinaryResult(Q, A) {
    this._binaryTokens.length > 0 && this._binaryTokens[this._binaryTokens.length - 2] === A - 1 && (this._binaryTokens.pop(), this._binaryTokens.pop()), this._binaryTokens.length === 0 && (this._lastTokenEndIndex = -1, this.produce(Q, A), this._binaryTokens[this._binaryTokens.length - 2] = 0);
    const B = new Uint32Array(this._binaryTokens.length);
    for (let g = 0, E = this._binaryTokens.length; g < E; g++)
      B[g] = this._binaryTokens[g];
    return B;
  }
}, Ug = class {
  constructor(Q, A) {
    c(this, "_grammars", /* @__PURE__ */ new Map());
    c(this, "_rawGrammars", /* @__PURE__ */ new Map());
    c(this, "_injectionGrammars", /* @__PURE__ */ new Map());
    c(this, "_theme");
    this._onigLib = A, this._theme = Q;
  }
  dispose() {
    for (const Q of this._grammars.values())
      Q.dispose();
  }
  setTheme(Q) {
    this._theme = Q;
  }
  getColorMap() {
    return this._theme.getColorMap();
  }
  /**
   * Add `grammar` to registry and return a list of referenced scope names
   */
  addGrammar(Q, A) {
    this._rawGrammars.set(Q.scopeName, Q), A && this._injectionGrammars.set(Q.scopeName, A);
  }
  /**
   * Lookup a raw grammar.
   */
  lookup(Q) {
    return this._rawGrammars.get(Q);
  }
  /**
   * Returns the injections for the given grammar
   */
  injections(Q) {
    return this._injectionGrammars.get(Q);
  }
  /**
   * Get the default theme settings
   */
  getDefaults() {
    return this._theme.getDefaults();
  }
  /**
   * Match a scope in the theme.
   */
  themeMatch(Q) {
    return this._theme.match(Q);
  }
  /**
   * Lookup a grammar.
   */
  grammarForScopeName(Q, A, B, g, E) {
    if (!this._grammars.has(Q)) {
      let C = this._rawGrammars.get(Q);
      if (!C)
        return null;
      this._grammars.set(Q, Hg(
        Q,
        C,
        A,
        B,
        g,
        E,
        this,
        this._onigLib
      ));
    }
    return this._grammars.get(Q);
  }
}, Yg = class {
  constructor(A) {
    c(this, "_options");
    c(this, "_syncRegistry");
    c(this, "_ensureGrammarCache");
    this._options = A, this._syncRegistry = new Ug(
      FA.createFromRawTheme(A.theme, A.colorMap),
      A.onigLib
    ), this._ensureGrammarCache = /* @__PURE__ */ new Map();
  }
  dispose() {
    this._syncRegistry.dispose();
  }
  /**
   * Change the theme. Once called, no previous `ruleStack` should be used anymore.
   */
  setTheme(A, B) {
    this._syncRegistry.setTheme(FA.createFromRawTheme(A, B));
  }
  /**
   * Returns a lookup array for color ids.
   */
  getColorMap() {
    return this._syncRegistry.getColorMap();
  }
  /**
   * Load the grammar for `scopeName` and all referenced included grammars asynchronously.
   * Please do not use language id 0.
   */
  loadGrammarWithEmbeddedLanguages(A, B, g) {
    return this.loadGrammarWithConfiguration(A, B, { embeddedLanguages: g });
  }
  /**
   * Load the grammar for `scopeName` and all referenced included grammars asynchronously.
   * Please do not use language id 0.
   */
  loadGrammarWithConfiguration(A, B, g) {
    return this._loadGrammar(
      A,
      B,
      g.embeddedLanguages,
      g.tokenTypes,
      new lg(
        g.balancedBracketSelectors || [],
        g.unbalancedBracketSelectors || []
      )
    );
  }
  /**
   * Load the grammar for `scopeName` and all referenced included grammars asynchronously.
   */
  loadGrammar(A) {
    return this._loadGrammar(A, 0, null, null, null);
  }
  _loadGrammar(A, B, g, E, C) {
    const I = new Cg(this._syncRegistry, A);
    for (; I.Q.length > 0; )
      I.Q.map((w) => this._loadSingleGrammar(w.scopeName)), I.processQueue();
    return this._grammarForScopeName(
      A,
      B,
      g,
      E,
      C
    );
  }
  _loadSingleGrammar(A) {
    this._ensureGrammarCache.has(A) || (this._doLoadSingleGrammar(A), this._ensureGrammarCache.set(A, !0));
  }
  _doLoadSingleGrammar(A) {
    const B = this._options.loadGrammar(A);
    if (B) {
      const g = typeof this._options.getInjections == "function" ? this._options.getInjections(A) : void 0;
      this._syncRegistry.addGrammar(B, g);
    }
  }
  /**
   * Adds a rawGrammar.
   */
  addGrammar(A, B = [], g = 0, E = null) {
    return this._syncRegistry.addGrammar(A, B), this._grammarForScopeName(A.scopeName, g, E);
  }
  /**
   * Get the grammar for `scopeName`. The grammar must first be created via `loadGrammar` or `addGrammar`.
   */
  _grammarForScopeName(A, B = 0, g = null, E = null, C = null) {
    return this._syncRegistry.grammarForScopeName(
      A,
      B,
      g,
      E,
      C
    );
  }
}, jA = zA.NULL;
const Pg = [
  "area",
  "base",
  "basefont",
  "bgsound",
  "br",
  "col",
  "command",
  "embed",
  "frame",
  "hr",
  "image",
  "img",
  "input",
  "keygen",
  "link",
  "meta",
  "param",
  "source",
  "track",
  "wbr"
];
class eA {
  /**
   * @param {SchemaType['property']} property
   *   Property.
   * @param {SchemaType['normal']} normal
   *   Normal.
   * @param {Space | undefined} [space]
   *   Space.
   * @returns
   *   Schema.
   */
  constructor(A, B, g) {
    this.normal = B, this.property = A, g && (this.space = g);
  }
}
eA.prototype.normal = {};
eA.prototype.property = {};
eA.prototype.space = void 0;
function CQ(Q, A) {
  const B = {}, g = {};
  for (const E of Q)
    Object.assign(B, E.property), Object.assign(g, E.normal);
  return new eA(B, g, A);
}
function _A(Q) {
  return Q.toLowerCase();
}
class S {
  /**
   * @param {string} property
   *   Property.
   * @param {string} attribute
   *   Attribute.
   * @returns
   *   Info.
   */
  constructor(A, B) {
    this.attribute = B, this.property = A;
  }
}
S.prototype.attribute = "";
S.prototype.booleanish = !1;
S.prototype.boolean = !1;
S.prototype.commaOrSpaceSeparated = !1;
S.prototype.commaSeparated = !1;
S.prototype.defined = !1;
S.prototype.mustUseProperty = !1;
S.prototype.number = !1;
S.prototype.overloadedBoolean = !1;
S.prototype.property = "";
S.prototype.spaceSeparated = !1;
S.prototype.space = void 0;
let ug = 0;
const r = v(), k = v(), $A = v(), M = v(), H = v(), z = v(), p = v();
function v() {
  return 2 ** ++ug;
}
const AB = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  boolean: r,
  booleanish: k,
  commaOrSpaceSeparated: p,
  commaSeparated: z,
  number: M,
  overloadedBoolean: $A,
  spaceSeparated: H
}, Symbol.toStringTag, { value: "Module" })), mA = (
  /** @type {ReadonlyArray<keyof typeof types>} */
  Object.keys(AB)
);
class CB extends S {
  /**
   * @constructor
   * @param {string} property
   *   Property.
   * @param {string} attribute
   *   Attribute.
   * @param {number | null | undefined} [mask]
   *   Mask.
   * @param {Space | undefined} [space]
   *   Space.
   * @returns
   *   Info.
   */
  constructor(A, B, g, E) {
    let C = -1;
    if (super(A, B), RB(this, "space", E), typeof g == "number")
      for (; ++C < mA.length; ) {
        const I = mA[C];
        RB(this, mA[C], (g & AB[I]) === AB[I]);
      }
  }
}
CB.prototype.defined = !0;
function RB(Q, A, B) {
  B && (Q[A] = B);
}
function AA(Q) {
  const A = {}, B = {};
  for (const [g, E] of Object.entries(Q.properties)) {
    const C = new CB(
      g,
      Q.transform(Q.attributes || {}, g),
      E,
      Q.space
    );
    Q.mustUseProperty && Q.mustUseProperty.includes(g) && (C.mustUseProperty = !0), A[g] = C, B[_A(g)] = g, B[_A(C.attribute)] = g;
  }
  return new eA(A, B, Q.space);
}
const IQ = AA({
  properties: {
    ariaActiveDescendant: null,
    ariaAtomic: k,
    ariaAutoComplete: null,
    ariaBusy: k,
    ariaChecked: k,
    ariaColCount: M,
    ariaColIndex: M,
    ariaColSpan: M,
    ariaControls: H,
    ariaCurrent: null,
    ariaDescribedBy: H,
    ariaDetails: null,
    ariaDisabled: k,
    ariaDropEffect: H,
    ariaErrorMessage: null,
    ariaExpanded: k,
    ariaFlowTo: H,
    ariaGrabbed: k,
    ariaHasPopup: null,
    ariaHidden: k,
    ariaInvalid: null,
    ariaKeyShortcuts: null,
    ariaLabel: null,
    ariaLabelledBy: H,
    ariaLevel: M,
    ariaLive: null,
    ariaModal: k,
    ariaMultiLine: k,
    ariaMultiSelectable: k,
    ariaOrientation: null,
    ariaOwns: H,
    ariaPlaceholder: null,
    ariaPosInSet: M,
    ariaPressed: k,
    ariaReadOnly: k,
    ariaRelevant: null,
    ariaRequired: k,
    ariaRoleDescription: H,
    ariaRowCount: M,
    ariaRowIndex: M,
    ariaRowSpan: M,
    ariaSelected: k,
    ariaSetSize: M,
    ariaSort: null,
    ariaValueMax: M,
    ariaValueMin: M,
    ariaValueNow: M,
    ariaValueText: null,
    role: null
  },
  transform(Q, A) {
    return A === "role" ? A : "aria-" + A.slice(4).toLowerCase();
  }
});
function DQ(Q, A) {
  return A in Q ? Q[A] : A;
}
function wQ(Q, A) {
  return DQ(Q, A.toLowerCase());
}
const yg = AA({
  attributes: {
    acceptcharset: "accept-charset",
    classname: "class",
    htmlfor: "for",
    httpequiv: "http-equiv"
  },
  mustUseProperty: ["checked", "multiple", "muted", "selected"],
  properties: {
    // Standard Properties.
    abbr: null,
    accept: z,
    acceptCharset: H,
    accessKey: H,
    action: null,
    allow: null,
    allowFullScreen: r,
    allowPaymentRequest: r,
    allowUserMedia: r,
    alt: null,
    as: null,
    async: r,
    autoCapitalize: null,
    autoComplete: H,
    autoFocus: r,
    autoPlay: r,
    blocking: H,
    capture: null,
    charSet: null,
    checked: r,
    cite: null,
    className: H,
    cols: M,
    colSpan: null,
    content: null,
    contentEditable: k,
    controls: r,
    controlsList: H,
    coords: M | z,
    crossOrigin: null,
    data: null,
    dateTime: null,
    decoding: null,
    default: r,
    defer: r,
    dir: null,
    dirName: null,
    disabled: r,
    download: $A,
    draggable: k,
    encType: null,
    enterKeyHint: null,
    fetchPriority: null,
    form: null,
    formAction: null,
    formEncType: null,
    formMethod: null,
    formNoValidate: r,
    formTarget: null,
    headers: H,
    height: M,
    hidden: $A,
    high: M,
    href: null,
    hrefLang: null,
    htmlFor: H,
    httpEquiv: H,
    id: null,
    imageSizes: null,
    imageSrcSet: null,
    inert: r,
    inputMode: null,
    integrity: null,
    is: null,
    isMap: r,
    itemId: null,
    itemProp: H,
    itemRef: H,
    itemScope: r,
    itemType: H,
    kind: null,
    label: null,
    lang: null,
    language: null,
    list: null,
    loading: null,
    loop: r,
    low: M,
    manifest: null,
    max: null,
    maxLength: M,
    media: null,
    method: null,
    min: null,
    minLength: M,
    multiple: r,
    muted: r,
    name: null,
    nonce: null,
    noModule: r,
    noValidate: r,
    onAbort: null,
    onAfterPrint: null,
    onAuxClick: null,
    onBeforeMatch: null,
    onBeforePrint: null,
    onBeforeToggle: null,
    onBeforeUnload: null,
    onBlur: null,
    onCancel: null,
    onCanPlay: null,
    onCanPlayThrough: null,
    onChange: null,
    onClick: null,
    onClose: null,
    onContextLost: null,
    onContextMenu: null,
    onContextRestored: null,
    onCopy: null,
    onCueChange: null,
    onCut: null,
    onDblClick: null,
    onDrag: null,
    onDragEnd: null,
    onDragEnter: null,
    onDragExit: null,
    onDragLeave: null,
    onDragOver: null,
    onDragStart: null,
    onDrop: null,
    onDurationChange: null,
    onEmptied: null,
    onEnded: null,
    onError: null,
    onFocus: null,
    onFormData: null,
    onHashChange: null,
    onInput: null,
    onInvalid: null,
    onKeyDown: null,
    onKeyPress: null,
    onKeyUp: null,
    onLanguageChange: null,
    onLoad: null,
    onLoadedData: null,
    onLoadedMetadata: null,
    onLoadEnd: null,
    onLoadStart: null,
    onMessage: null,
    onMessageError: null,
    onMouseDown: null,
    onMouseEnter: null,
    onMouseLeave: null,
    onMouseMove: null,
    onMouseOut: null,
    onMouseOver: null,
    onMouseUp: null,
    onOffline: null,
    onOnline: null,
    onPageHide: null,
    onPageShow: null,
    onPaste: null,
    onPause: null,
    onPlay: null,
    onPlaying: null,
    onPopState: null,
    onProgress: null,
    onRateChange: null,
    onRejectionHandled: null,
    onReset: null,
    onResize: null,
    onScroll: null,
    onScrollEnd: null,
    onSecurityPolicyViolation: null,
    onSeeked: null,
    onSeeking: null,
    onSelect: null,
    onSlotChange: null,
    onStalled: null,
    onStorage: null,
    onSubmit: null,
    onSuspend: null,
    onTimeUpdate: null,
    onToggle: null,
    onUnhandledRejection: null,
    onUnload: null,
    onVolumeChange: null,
    onWaiting: null,
    onWheel: null,
    open: r,
    optimum: M,
    pattern: null,
    ping: H,
    placeholder: null,
    playsInline: r,
    popover: null,
    popoverTarget: null,
    popoverTargetAction: null,
    poster: null,
    preload: null,
    readOnly: r,
    referrerPolicy: null,
    rel: H,
    required: r,
    reversed: r,
    rows: M,
    rowSpan: M,
    sandbox: H,
    scope: null,
    scoped: r,
    seamless: r,
    selected: r,
    shadowRootClonable: r,
    shadowRootDelegatesFocus: r,
    shadowRootMode: null,
    shape: null,
    size: M,
    sizes: null,
    slot: null,
    span: M,
    spellCheck: k,
    src: null,
    srcDoc: null,
    srcLang: null,
    srcSet: null,
    start: M,
    step: null,
    style: null,
    tabIndex: M,
    target: null,
    title: null,
    translate: null,
    type: null,
    typeMustMatch: r,
    useMap: null,
    value: k,
    width: M,
    wrap: null,
    writingSuggestions: null,
    // Legacy.
    // See: https://html.spec.whatwg.org/#other-elements,-attributes-and-apis
    align: null,
    // Several. Use CSS `text-align` instead,
    aLink: null,
    // `<body>`. Use CSS `a:active {color}` instead
    archive: H,
    // `<object>`. List of URIs to archives
    axis: null,
    // `<td>` and `<th>`. Use `scope` on `<th>`
    background: null,
    // `<body>`. Use CSS `background-image` instead
    bgColor: null,
    // `<body>` and table elements. Use CSS `background-color` instead
    border: M,
    // `<table>`. Use CSS `border-width` instead,
    borderColor: null,
    // `<table>`. Use CSS `border-color` instead,
    bottomMargin: M,
    // `<body>`
    cellPadding: null,
    // `<table>`
    cellSpacing: null,
    // `<table>`
    char: null,
    // Several table elements. When `align=char`, sets the character to align on
    charOff: null,
    // Several table elements. When `char`, offsets the alignment
    classId: null,
    // `<object>`
    clear: null,
    // `<br>`. Use CSS `clear` instead
    code: null,
    // `<object>`
    codeBase: null,
    // `<object>`
    codeType: null,
    // `<object>`
    color: null,
    // `<font>` and `<hr>`. Use CSS instead
    compact: r,
    // Lists. Use CSS to reduce space between items instead
    declare: r,
    // `<object>`
    event: null,
    // `<script>`
    face: null,
    // `<font>`. Use CSS instead
    frame: null,
    // `<table>`
    frameBorder: null,
    // `<iframe>`. Use CSS `border` instead
    hSpace: M,
    // `<img>` and `<object>`
    leftMargin: M,
    // `<body>`
    link: null,
    // `<body>`. Use CSS `a:link {color: *}` instead
    longDesc: null,
    // `<frame>`, `<iframe>`, and `<img>`. Use an `<a>`
    lowSrc: null,
    // `<img>`. Use a `<picture>`
    marginHeight: M,
    // `<body>`
    marginWidth: M,
    // `<body>`
    noResize: r,
    // `<frame>`
    noHref: r,
    // `<area>`. Use no href instead of an explicit `nohref`
    noShade: r,
    // `<hr>`. Use background-color and height instead of borders
    noWrap: r,
    // `<td>` and `<th>`
    object: null,
    // `<applet>`
    profile: null,
    // `<head>`
    prompt: null,
    // `<isindex>`
    rev: null,
    // `<link>`
    rightMargin: M,
    // `<body>`
    rules: null,
    // `<table>`
    scheme: null,
    // `<meta>`
    scrolling: k,
    // `<frame>`. Use overflow in the child context
    standby: null,
    // `<object>`
    summary: null,
    // `<table>`
    text: null,
    // `<body>`. Use CSS `color` instead
    topMargin: M,
    // `<body>`
    valueType: null,
    // `<param>`
    version: null,
    // `<html>`. Use a doctype.
    vAlign: null,
    // Several. Use CSS `vertical-align` instead
    vLink: null,
    // `<body>`. Use CSS `a:visited {color}` instead
    vSpace: M,
    // `<img>` and `<object>`
    // Non-standard Properties.
    allowTransparency: null,
    autoCorrect: null,
    autoSave: null,
    disablePictureInPicture: r,
    disableRemotePlayback: r,
    prefix: null,
    property: null,
    results: M,
    security: null,
    unselectable: null
  },
  space: "html",
  transform: wQ
}), Jg = AA({
  attributes: {
    accentHeight: "accent-height",
    alignmentBaseline: "alignment-baseline",
    arabicForm: "arabic-form",
    baselineShift: "baseline-shift",
    capHeight: "cap-height",
    className: "class",
    clipPath: "clip-path",
    clipRule: "clip-rule",
    colorInterpolation: "color-interpolation",
    colorInterpolationFilters: "color-interpolation-filters",
    colorProfile: "color-profile",
    colorRendering: "color-rendering",
    crossOrigin: "crossorigin",
    dataType: "datatype",
    dominantBaseline: "dominant-baseline",
    enableBackground: "enable-background",
    fillOpacity: "fill-opacity",
    fillRule: "fill-rule",
    floodColor: "flood-color",
    floodOpacity: "flood-opacity",
    fontFamily: "font-family",
    fontSize: "font-size",
    fontSizeAdjust: "font-size-adjust",
    fontStretch: "font-stretch",
    fontStyle: "font-style",
    fontVariant: "font-variant",
    fontWeight: "font-weight",
    glyphName: "glyph-name",
    glyphOrientationHorizontal: "glyph-orientation-horizontal",
    glyphOrientationVertical: "glyph-orientation-vertical",
    hrefLang: "hreflang",
    horizAdvX: "horiz-adv-x",
    horizOriginX: "horiz-origin-x",
    horizOriginY: "horiz-origin-y",
    imageRendering: "image-rendering",
    letterSpacing: "letter-spacing",
    lightingColor: "lighting-color",
    markerEnd: "marker-end",
    markerMid: "marker-mid",
    markerStart: "marker-start",
    navDown: "nav-down",
    navDownLeft: "nav-down-left",
    navDownRight: "nav-down-right",
    navLeft: "nav-left",
    navNext: "nav-next",
    navPrev: "nav-prev",
    navRight: "nav-right",
    navUp: "nav-up",
    navUpLeft: "nav-up-left",
    navUpRight: "nav-up-right",
    onAbort: "onabort",
    onActivate: "onactivate",
    onAfterPrint: "onafterprint",
    onBeforePrint: "onbeforeprint",
    onBegin: "onbegin",
    onCancel: "oncancel",
    onCanPlay: "oncanplay",
    onCanPlayThrough: "oncanplaythrough",
    onChange: "onchange",
    onClick: "onclick",
    onClose: "onclose",
    onCopy: "oncopy",
    onCueChange: "oncuechange",
    onCut: "oncut",
    onDblClick: "ondblclick",
    onDrag: "ondrag",
    onDragEnd: "ondragend",
    onDragEnter: "ondragenter",
    onDragExit: "ondragexit",
    onDragLeave: "ondragleave",
    onDragOver: "ondragover",
    onDragStart: "ondragstart",
    onDrop: "ondrop",
    onDurationChange: "ondurationchange",
    onEmptied: "onemptied",
    onEnd: "onend",
    onEnded: "onended",
    onError: "onerror",
    onFocus: "onfocus",
    onFocusIn: "onfocusin",
    onFocusOut: "onfocusout",
    onHashChange: "onhashchange",
    onInput: "oninput",
    onInvalid: "oninvalid",
    onKeyDown: "onkeydown",
    onKeyPress: "onkeypress",
    onKeyUp: "onkeyup",
    onLoad: "onload",
    onLoadedData: "onloadeddata",
    onLoadedMetadata: "onloadedmetadata",
    onLoadStart: "onloadstart",
    onMessage: "onmessage",
    onMouseDown: "onmousedown",
    onMouseEnter: "onmouseenter",
    onMouseLeave: "onmouseleave",
    onMouseMove: "onmousemove",
    onMouseOut: "onmouseout",
    onMouseOver: "onmouseover",
    onMouseUp: "onmouseup",
    onMouseWheel: "onmousewheel",
    onOffline: "onoffline",
    onOnline: "ononline",
    onPageHide: "onpagehide",
    onPageShow: "onpageshow",
    onPaste: "onpaste",
    onPause: "onpause",
    onPlay: "onplay",
    onPlaying: "onplaying",
    onPopState: "onpopstate",
    onProgress: "onprogress",
    onRateChange: "onratechange",
    onRepeat: "onrepeat",
    onReset: "onreset",
    onResize: "onresize",
    onScroll: "onscroll",
    onSeeked: "onseeked",
    onSeeking: "onseeking",
    onSelect: "onselect",
    onShow: "onshow",
    onStalled: "onstalled",
    onStorage: "onstorage",
    onSubmit: "onsubmit",
    onSuspend: "onsuspend",
    onTimeUpdate: "ontimeupdate",
    onToggle: "ontoggle",
    onUnload: "onunload",
    onVolumeChange: "onvolumechange",
    onWaiting: "onwaiting",
    onZoom: "onzoom",
    overlinePosition: "overline-position",
    overlineThickness: "overline-thickness",
    paintOrder: "paint-order",
    panose1: "panose-1",
    pointerEvents: "pointer-events",
    referrerPolicy: "referrerpolicy",
    renderingIntent: "rendering-intent",
    shapeRendering: "shape-rendering",
    stopColor: "stop-color",
    stopOpacity: "stop-opacity",
    strikethroughPosition: "strikethrough-position",
    strikethroughThickness: "strikethrough-thickness",
    strokeDashArray: "stroke-dasharray",
    strokeDashOffset: "stroke-dashoffset",
    strokeLineCap: "stroke-linecap",
    strokeLineJoin: "stroke-linejoin",
    strokeMiterLimit: "stroke-miterlimit",
    strokeOpacity: "stroke-opacity",
    strokeWidth: "stroke-width",
    tabIndex: "tabindex",
    textAnchor: "text-anchor",
    textDecoration: "text-decoration",
    textRendering: "text-rendering",
    transformOrigin: "transform-origin",
    typeOf: "typeof",
    underlinePosition: "underline-position",
    underlineThickness: "underline-thickness",
    unicodeBidi: "unicode-bidi",
    unicodeRange: "unicode-range",
    unitsPerEm: "units-per-em",
    vAlphabetic: "v-alphabetic",
    vHanging: "v-hanging",
    vIdeographic: "v-ideographic",
    vMathematical: "v-mathematical",
    vectorEffect: "vector-effect",
    vertAdvY: "vert-adv-y",
    vertOriginX: "vert-origin-x",
    vertOriginY: "vert-origin-y",
    wordSpacing: "word-spacing",
    writingMode: "writing-mode",
    xHeight: "x-height",
    // These were camelcased in Tiny. Now lowercased in SVG 2
    playbackOrder: "playbackorder",
    timelineBegin: "timelinebegin"
  },
  properties: {
    about: p,
    accentHeight: M,
    accumulate: null,
    additive: null,
    alignmentBaseline: null,
    alphabetic: M,
    amplitude: M,
    arabicForm: null,
    ascent: M,
    attributeName: null,
    attributeType: null,
    azimuth: M,
    bandwidth: null,
    baselineShift: null,
    baseFrequency: null,
    baseProfile: null,
    bbox: null,
    begin: null,
    bias: M,
    by: null,
    calcMode: null,
    capHeight: M,
    className: H,
    clip: null,
    clipPath: null,
    clipPathUnits: null,
    clipRule: null,
    color: null,
    colorInterpolation: null,
    colorInterpolationFilters: null,
    colorProfile: null,
    colorRendering: null,
    content: null,
    contentScriptType: null,
    contentStyleType: null,
    crossOrigin: null,
    cursor: null,
    cx: null,
    cy: null,
    d: null,
    dataType: null,
    defaultAction: null,
    descent: M,
    diffuseConstant: M,
    direction: null,
    display: null,
    dur: null,
    divisor: M,
    dominantBaseline: null,
    download: r,
    dx: null,
    dy: null,
    edgeMode: null,
    editable: null,
    elevation: M,
    enableBackground: null,
    end: null,
    event: null,
    exponent: M,
    externalResourcesRequired: null,
    fill: null,
    fillOpacity: M,
    fillRule: null,
    filter: null,
    filterRes: null,
    filterUnits: null,
    floodColor: null,
    floodOpacity: null,
    focusable: null,
    focusHighlight: null,
    fontFamily: null,
    fontSize: null,
    fontSizeAdjust: null,
    fontStretch: null,
    fontStyle: null,
    fontVariant: null,
    fontWeight: null,
    format: null,
    fr: null,
    from: null,
    fx: null,
    fy: null,
    g1: z,
    g2: z,
    glyphName: z,
    glyphOrientationHorizontal: null,
    glyphOrientationVertical: null,
    glyphRef: null,
    gradientTransform: null,
    gradientUnits: null,
    handler: null,
    hanging: M,
    hatchContentUnits: null,
    hatchUnits: null,
    height: null,
    href: null,
    hrefLang: null,
    horizAdvX: M,
    horizOriginX: M,
    horizOriginY: M,
    id: null,
    ideographic: M,
    imageRendering: null,
    initialVisibility: null,
    in: null,
    in2: null,
    intercept: M,
    k: M,
    k1: M,
    k2: M,
    k3: M,
    k4: M,
    kernelMatrix: p,
    kernelUnitLength: null,
    keyPoints: null,
    // SEMI_COLON_SEPARATED
    keySplines: null,
    // SEMI_COLON_SEPARATED
    keyTimes: null,
    // SEMI_COLON_SEPARATED
    kerning: null,
    lang: null,
    lengthAdjust: null,
    letterSpacing: null,
    lightingColor: null,
    limitingConeAngle: M,
    local: null,
    markerEnd: null,
    markerMid: null,
    markerStart: null,
    markerHeight: null,
    markerUnits: null,
    markerWidth: null,
    mask: null,
    maskContentUnits: null,
    maskUnits: null,
    mathematical: null,
    max: null,
    media: null,
    mediaCharacterEncoding: null,
    mediaContentEncodings: null,
    mediaSize: M,
    mediaTime: null,
    method: null,
    min: null,
    mode: null,
    name: null,
    navDown: null,
    navDownLeft: null,
    navDownRight: null,
    navLeft: null,
    navNext: null,
    navPrev: null,
    navRight: null,
    navUp: null,
    navUpLeft: null,
    navUpRight: null,
    numOctaves: null,
    observer: null,
    offset: null,
    onAbort: null,
    onActivate: null,
    onAfterPrint: null,
    onBeforePrint: null,
    onBegin: null,
    onCancel: null,
    onCanPlay: null,
    onCanPlayThrough: null,
    onChange: null,
    onClick: null,
    onClose: null,
    onCopy: null,
    onCueChange: null,
    onCut: null,
    onDblClick: null,
    onDrag: null,
    onDragEnd: null,
    onDragEnter: null,
    onDragExit: null,
    onDragLeave: null,
    onDragOver: null,
    onDragStart: null,
    onDrop: null,
    onDurationChange: null,
    onEmptied: null,
    onEnd: null,
    onEnded: null,
    onError: null,
    onFocus: null,
    onFocusIn: null,
    onFocusOut: null,
    onHashChange: null,
    onInput: null,
    onInvalid: null,
    onKeyDown: null,
    onKeyPress: null,
    onKeyUp: null,
    onLoad: null,
    onLoadedData: null,
    onLoadedMetadata: null,
    onLoadStart: null,
    onMessage: null,
    onMouseDown: null,
    onMouseEnter: null,
    onMouseLeave: null,
    onMouseMove: null,
    onMouseOut: null,
    onMouseOver: null,
    onMouseUp: null,
    onMouseWheel: null,
    onOffline: null,
    onOnline: null,
    onPageHide: null,
    onPageShow: null,
    onPaste: null,
    onPause: null,
    onPlay: null,
    onPlaying: null,
    onPopState: null,
    onProgress: null,
    onRateChange: null,
    onRepeat: null,
    onReset: null,
    onResize: null,
    onScroll: null,
    onSeeked: null,
    onSeeking: null,
    onSelect: null,
    onShow: null,
    onStalled: null,
    onStorage: null,
    onSubmit: null,
    onSuspend: null,
    onTimeUpdate: null,
    onToggle: null,
    onUnload: null,
    onVolumeChange: null,
    onWaiting: null,
    onZoom: null,
    opacity: null,
    operator: null,
    order: null,
    orient: null,
    orientation: null,
    origin: null,
    overflow: null,
    overlay: null,
    overlinePosition: M,
    overlineThickness: M,
    paintOrder: null,
    panose1: null,
    path: null,
    pathLength: M,
    patternContentUnits: null,
    patternTransform: null,
    patternUnits: null,
    phase: null,
    ping: H,
    pitch: null,
    playbackOrder: null,
    pointerEvents: null,
    points: null,
    pointsAtX: M,
    pointsAtY: M,
    pointsAtZ: M,
    preserveAlpha: null,
    preserveAspectRatio: null,
    primitiveUnits: null,
    propagate: null,
    property: p,
    r: null,
    radius: null,
    referrerPolicy: null,
    refX: null,
    refY: null,
    rel: p,
    rev: p,
    renderingIntent: null,
    repeatCount: null,
    repeatDur: null,
    requiredExtensions: p,
    requiredFeatures: p,
    requiredFonts: p,
    requiredFormats: p,
    resource: null,
    restart: null,
    result: null,
    rotate: null,
    rx: null,
    ry: null,
    scale: null,
    seed: null,
    shapeRendering: null,
    side: null,
    slope: null,
    snapshotTime: null,
    specularConstant: M,
    specularExponent: M,
    spreadMethod: null,
    spacing: null,
    startOffset: null,
    stdDeviation: null,
    stemh: null,
    stemv: null,
    stitchTiles: null,
    stopColor: null,
    stopOpacity: null,
    strikethroughPosition: M,
    strikethroughThickness: M,
    string: null,
    stroke: null,
    strokeDashArray: p,
    strokeDashOffset: null,
    strokeLineCap: null,
    strokeLineJoin: null,
    strokeMiterLimit: M,
    strokeOpacity: M,
    strokeWidth: null,
    style: null,
    surfaceScale: M,
    syncBehavior: null,
    syncBehaviorDefault: null,
    syncMaster: null,
    syncTolerance: null,
    syncToleranceDefault: null,
    systemLanguage: p,
    tabIndex: M,
    tableValues: null,
    target: null,
    targetX: M,
    targetY: M,
    textAnchor: null,
    textDecoration: null,
    textRendering: null,
    textLength: null,
    timelineBegin: null,
    title: null,
    transformBehavior: null,
    type: null,
    typeOf: p,
    to: null,
    transform: null,
    transformOrigin: null,
    u1: null,
    u2: null,
    underlinePosition: M,
    underlineThickness: M,
    unicode: null,
    unicodeBidi: null,
    unicodeRange: null,
    unitsPerEm: M,
    values: null,
    vAlphabetic: M,
    vMathematical: M,
    vectorEffect: null,
    vHanging: M,
    vIdeographic: M,
    version: null,
    vertAdvY: M,
    vertOriginX: M,
    vertOriginY: M,
    viewBox: null,
    viewTarget: null,
    visibility: null,
    width: null,
    widths: null,
    wordSpacing: null,
    writingMode: null,
    x: null,
    x1: null,
    x2: null,
    xChannelSelector: null,
    xHeight: M,
    y: null,
    y1: null,
    y2: null,
    yChannelSelector: null,
    z: null,
    zoomAndPan: null
  },
  space: "svg",
  transform: DQ
}), oQ = AA({
  properties: {
    xLinkActuate: null,
    xLinkArcRole: null,
    xLinkHref: null,
    xLinkRole: null,
    xLinkShow: null,
    xLinkTitle: null,
    xLinkType: null
  },
  space: "xlink",
  transform(Q, A) {
    return "xlink:" + A.slice(5).toLowerCase();
  }
}), sQ = AA({
  attributes: { xmlnsxlink: "xmlns:xlink" },
  properties: { xmlnsXLink: null, xmlns: null },
  space: "xmlns",
  transform: wQ
}), eQ = AA({
  properties: { xmlBase: null, xmlLang: null, xmlSpace: null },
  space: "xml",
  transform(Q, A) {
    return "xml:" + A.slice(3).toLowerCase();
  }
}), Sg = /[A-Z]/g, kB = /-[a-z]/g, pg = /^data[-\w.:]+$/i;
function fg(Q, A) {
  const B = _A(A);
  let g = A, E = S;
  if (B in Q.normal)
    return Q.property[Q.normal[B]];
  if (B.length > 4 && B.slice(0, 4) === "data" && pg.test(A)) {
    if (A.charAt(4) === "-") {
      const C = A.slice(5).replace(kB, Og);
      g = "data" + C.charAt(0).toUpperCase() + C.slice(1);
    } else {
      const C = A.slice(4);
      if (!kB.test(C)) {
        let I = C.replace(Sg, dg);
        I.charAt(0) !== "-" && (I = "-" + I), A = "data" + I;
      }
    }
    E = CB;
  }
  return new E(g, A);
}
function dg(Q) {
  return "-" + Q.toLowerCase();
}
function Og(Q) {
  return Q.charAt(1).toUpperCase();
}
const xg = CQ([IQ, yg, oQ, sQ, eQ], "html"), iQ = CQ([IQ, Jg, oQ, sQ, eQ], "svg"), lB = {}.hasOwnProperty;
function mg(Q, A) {
  const B = A || {};
  function g(E, ...C) {
    let I = g.invalid;
    const w = g.handlers;
    if (E && lB.call(E, Q)) {
      const D = String(E[Q]);
      I = lB.call(w, D) ? w[D] : g.unknown;
    }
    if (I)
      return I.call(this, E, ...C);
  }
  return g.handlers = B.handlers || {}, g.invalid = B.invalid, g.unknown = B.unknown, g;
}
const qg = /["&'<>`]/g, bg = /[\uD800-\uDBFF][\uDC00-\uDFFF]/g, Tg = (
  // eslint-disable-next-line no-control-regex, unicorn/no-hex-escape
  /[\x01-\t\v\f\x0E-\x1F\x7F\x81\x8D\x8F\x90\x9D\xA0-\uFFFF]/g
), Wg = /[|\\{}()[\]^$+*?.]/g, KB = /* @__PURE__ */ new WeakMap();
function vg(Q, A) {
  if (Q = Q.replace(
    A.subset ? Xg(A.subset) : qg,
    g
  ), A.subset || A.escapeOnly)
    return Q;
  return Q.replace(bg, B).replace(Tg, g);
  function B(E, C, I) {
    return A.format(
      (E.charCodeAt(0) - 55296) * 1024 + E.charCodeAt(1) - 56320 + 65536,
      I.charCodeAt(C + 2),
      A
    );
  }
  function g(E, C, I) {
    return A.format(
      E.charCodeAt(0),
      I.charCodeAt(C + 1),
      A
    );
  }
}
function Xg(Q) {
  let A = KB.get(Q);
  return A || (A = Zg(Q), KB.set(Q, A)), A;
}
function Zg(Q) {
  const A = [];
  let B = -1;
  for (; ++B < Q.length; )
    A.push(Q[B].replace(Wg, "\\$&"));
  return new RegExp("(?:" + A.join("|") + ")", "g");
}
const Vg = /[\dA-Fa-f]/;
function zg(Q, A, B) {
  const g = "&#x" + Q.toString(16).toUpperCase();
  return B && A && !Vg.test(String.fromCharCode(A)) ? g : g + ";";
}
const jg = /\d/;
function _g(Q, A, B) {
  const g = "&#" + String(Q);
  return B && A && !jg.test(String.fromCharCode(A)) ? g : g + ";";
}
const $g = [
  "AElig",
  "AMP",
  "Aacute",
  "Acirc",
  "Agrave",
  "Aring",
  "Atilde",
  "Auml",
  "COPY",
  "Ccedil",
  "ETH",
  "Eacute",
  "Ecirc",
  "Egrave",
  "Euml",
  "GT",
  "Iacute",
  "Icirc",
  "Igrave",
  "Iuml",
  "LT",
  "Ntilde",
  "Oacute",
  "Ocirc",
  "Ograve",
  "Oslash",
  "Otilde",
  "Ouml",
  "QUOT",
  "REG",
  "THORN",
  "Uacute",
  "Ucirc",
  "Ugrave",
  "Uuml",
  "Yacute",
  "aacute",
  "acirc",
  "acute",
  "aelig",
  "agrave",
  "amp",
  "aring",
  "atilde",
  "auml",
  "brvbar",
  "ccedil",
  "cedil",
  "cent",
  "copy",
  "curren",
  "deg",
  "divide",
  "eacute",
  "ecirc",
  "egrave",
  "eth",
  "euml",
  "frac12",
  "frac14",
  "frac34",
  "gt",
  "iacute",
  "icirc",
  "iexcl",
  "igrave",
  "iquest",
  "iuml",
  "laquo",
  "lt",
  "macr",
  "micro",
  "middot",
  "nbsp",
  "not",
  "ntilde",
  "oacute",
  "ocirc",
  "ograve",
  "ordf",
  "ordm",
  "oslash",
  "otilde",
  "ouml",
  "para",
  "plusmn",
  "pound",
  "quot",
  "raquo",
  "reg",
  "sect",
  "shy",
  "sup1",
  "sup2",
  "sup3",
  "szlig",
  "thorn",
  "times",
  "uacute",
  "ucirc",
  "ugrave",
  "uml",
  "uuml",
  "yacute",
  "yen",
  "yuml"
], qA = {
  nbsp: " ",
  iexcl: "¡",
  cent: "¢",
  pound: "£",
  curren: "¤",
  yen: "¥",
  brvbar: "¦",
  sect: "§",
  uml: "¨",
  copy: "©",
  ordf: "ª",
  laquo: "«",
  not: "¬",
  shy: "­",
  reg: "®",
  macr: "¯",
  deg: "°",
  plusmn: "±",
  sup2: "²",
  sup3: "³",
  acute: "´",
  micro: "µ",
  para: "¶",
  middot: "·",
  cedil: "¸",
  sup1: "¹",
  ordm: "º",
  raquo: "»",
  frac14: "¼",
  frac12: "½",
  frac34: "¾",
  iquest: "¿",
  Agrave: "À",
  Aacute: "Á",
  Acirc: "Â",
  Atilde: "Ã",
  Auml: "Ä",
  Aring: "Å",
  AElig: "Æ",
  Ccedil: "Ç",
  Egrave: "È",
  Eacute: "É",
  Ecirc: "Ê",
  Euml: "Ë",
  Igrave: "Ì",
  Iacute: "Í",
  Icirc: "Î",
  Iuml: "Ï",
  ETH: "Ð",
  Ntilde: "Ñ",
  Ograve: "Ò",
  Oacute: "Ó",
  Ocirc: "Ô",
  Otilde: "Õ",
  Ouml: "Ö",
  times: "×",
  Oslash: "Ø",
  Ugrave: "Ù",
  Uacute: "Ú",
  Ucirc: "Û",
  Uuml: "Ü",
  Yacute: "Ý",
  THORN: "Þ",
  szlig: "ß",
  agrave: "à",
  aacute: "á",
  acirc: "â",
  atilde: "ã",
  auml: "ä",
  aring: "å",
  aelig: "æ",
  ccedil: "ç",
  egrave: "è",
  eacute: "é",
  ecirc: "ê",
  euml: "ë",
  igrave: "ì",
  iacute: "í",
  icirc: "î",
  iuml: "ï",
  eth: "ð",
  ntilde: "ñ",
  ograve: "ò",
  oacute: "ó",
  ocirc: "ô",
  otilde: "õ",
  ouml: "ö",
  divide: "÷",
  oslash: "ø",
  ugrave: "ù",
  uacute: "ú",
  ucirc: "û",
  uuml: "ü",
  yacute: "ý",
  thorn: "þ",
  yuml: "ÿ",
  fnof: "ƒ",
  Alpha: "Α",
  Beta: "Β",
  Gamma: "Γ",
  Delta: "Δ",
  Epsilon: "Ε",
  Zeta: "Ζ",
  Eta: "Η",
  Theta: "Θ",
  Iota: "Ι",
  Kappa: "Κ",
  Lambda: "Λ",
  Mu: "Μ",
  Nu: "Ν",
  Xi: "Ξ",
  Omicron: "Ο",
  Pi: "Π",
  Rho: "Ρ",
  Sigma: "Σ",
  Tau: "Τ",
  Upsilon: "Υ",
  Phi: "Φ",
  Chi: "Χ",
  Psi: "Ψ",
  Omega: "Ω",
  alpha: "α",
  beta: "β",
  gamma: "γ",
  delta: "δ",
  epsilon: "ε",
  zeta: "ζ",
  eta: "η",
  theta: "θ",
  iota: "ι",
  kappa: "κ",
  lambda: "λ",
  mu: "μ",
  nu: "ν",
  xi: "ξ",
  omicron: "ο",
  pi: "π",
  rho: "ρ",
  sigmaf: "ς",
  sigma: "σ",
  tau: "τ",
  upsilon: "υ",
  phi: "φ",
  chi: "χ",
  psi: "ψ",
  omega: "ω",
  thetasym: "ϑ",
  upsih: "ϒ",
  piv: "ϖ",
  bull: "•",
  hellip: "…",
  prime: "′",
  Prime: "″",
  oline: "‾",
  frasl: "⁄",
  weierp: "℘",
  image: "ℑ",
  real: "ℜ",
  trade: "™",
  alefsym: "ℵ",
  larr: "←",
  uarr: "↑",
  rarr: "→",
  darr: "↓",
  harr: "↔",
  crarr: "↵",
  lArr: "⇐",
  uArr: "⇑",
  rArr: "⇒",
  dArr: "⇓",
  hArr: "⇔",
  forall: "∀",
  part: "∂",
  exist: "∃",
  empty: "∅",
  nabla: "∇",
  isin: "∈",
  notin: "∉",
  ni: "∋",
  prod: "∏",
  sum: "∑",
  minus: "−",
  lowast: "∗",
  radic: "√",
  prop: "∝",
  infin: "∞",
  ang: "∠",
  and: "∧",
  or: "∨",
  cap: "∩",
  cup: "∪",
  int: "∫",
  there4: "∴",
  sim: "∼",
  cong: "≅",
  asymp: "≈",
  ne: "≠",
  equiv: "≡",
  le: "≤",
  ge: "≥",
  sub: "⊂",
  sup: "⊃",
  nsub: "⊄",
  sube: "⊆",
  supe: "⊇",
  oplus: "⊕",
  otimes: "⊗",
  perp: "⊥",
  sdot: "⋅",
  lceil: "⌈",
  rceil: "⌉",
  lfloor: "⌊",
  rfloor: "⌋",
  lang: "〈",
  rang: "〉",
  loz: "◊",
  spades: "♠",
  clubs: "♣",
  hearts: "♥",
  diams: "♦",
  quot: '"',
  amp: "&",
  lt: "<",
  gt: ">",
  OElig: "Œ",
  oelig: "œ",
  Scaron: "Š",
  scaron: "š",
  Yuml: "Ÿ",
  circ: "ˆ",
  tilde: "˜",
  ensp: " ",
  emsp: " ",
  thinsp: " ",
  zwnj: "‌",
  zwj: "‍",
  lrm: "‎",
  rlm: "‏",
  ndash: "–",
  mdash: "—",
  lsquo: "‘",
  rsquo: "’",
  sbquo: "‚",
  ldquo: "“",
  rdquo: "”",
  bdquo: "„",
  dagger: "†",
  Dagger: "‡",
  permil: "‰",
  lsaquo: "‹",
  rsaquo: "›",
  euro: "€"
}, AE = [
  "cent",
  "copy",
  "divide",
  "gt",
  "lt",
  "not",
  "para",
  "times"
], tQ = {}.hasOwnProperty, BB = {};
let cA;
for (cA in qA)
  tQ.call(qA, cA) && (BB[qA[cA]] = cA);
const BE = /[^\dA-Za-z]/;
function QE(Q, A, B, g) {
  const E = String.fromCharCode(Q);
  if (tQ.call(BB, E)) {
    const C = BB[E], I = "&" + C;
    return B && $g.includes(C) && !AE.includes(C) && (!g || A && A !== 61 && BE.test(String.fromCharCode(A))) ? I : I + ";";
  }
  return "";
}
function gE(Q, A, B) {
  let g = zg(Q, A, B.omitOptionalSemicolons), E;
  if ((B.useNamedReferences || B.useShortestReferences) && (E = QE(
    Q,
    A,
    B.omitOptionalSemicolons,
    B.attribute
  )), (B.useShortestReferences || !E) && B.useShortestReferences) {
    const C = _g(Q, A, B.omitOptionalSemicolons);
    C.length < g.length && (g = C);
  }
  return E && (!B.useShortestReferences || E.length < g.length) ? E : g;
}
function j(Q, A) {
  return vg(Q, Object.assign({ format: gE }, A));
}
const EE = /^>|^->|<!--|-->|--!>|<!-$/g, CE = [">"], IE = ["<", ">"];
function DE(Q, A, B, g) {
  return g.settings.bogusComments ? "<?" + j(
    Q.value,
    Object.assign({}, g.settings.characterReferences, {
      subset: CE
    })
  ) + ">" : "<!--" + Q.value.replace(EE, E) + "-->";
  function E(C) {
    return j(
      C,
      Object.assign({}, g.settings.characterReferences, {
        subset: IE
      })
    );
  }
}
function wE(Q, A, B, g) {
  return "<!" + (g.settings.upperDoctype ? "DOCTYPE" : "doctype") + (g.settings.tightDoctype ? "" : " ") + "html>";
}
function UB(Q, A) {
  const B = String(Q);
  if (typeof A != "string")
    throw new TypeError("Expected character");
  let g = 0, E = B.indexOf(A);
  for (; E !== -1; )
    g++, E = B.indexOf(A, E + A.length);
  return g;
}
function oE(Q, A) {
  const B = A || {};
  return (Q[Q.length - 1] === "" ? [...Q, ""] : Q).join(
    (B.padRight ? " " : "") + "," + (B.padLeft === !1 ? "" : " ")
  ).trim();
}
function sE(Q) {
  return Q.join(" ").trim();
}
const eE = /[ \t\n\f\r]/g;
function IB(Q) {
  return typeof Q == "object" ? Q.type === "text" ? YB(Q.value) : !1 : YB(Q);
}
function YB(Q) {
  return Q.replace(eE, "") === "";
}
const K = nQ(1), cQ = nQ(-1), iE = [];
function nQ(Q) {
  return A;
  function A(B, g, E) {
    const C = B ? B.children : iE;
    let I = (g || 0) + Q, w = C[I];
    if (!E)
      for (; w && IB(w); )
        I += Q, w = C[I];
    return w;
  }
}
const tE = {}.hasOwnProperty;
function aQ(Q) {
  return A;
  function A(B, g, E) {
    return tE.call(Q, B.tagName) && Q[B.tagName](B, g, E);
  }
}
const DB = aQ({
  body: nE,
  caption: bA,
  colgroup: bA,
  dd: FE,
  dt: hE,
  head: bA,
  html: cE,
  li: ME,
  optgroup: GE,
  option: NE,
  p: aE,
  rp: PB,
  rt: PB,
  tbody: LE,
  td: uB,
  tfoot: HE,
  th: uB,
  thead: rE,
  tr: RE
});
function bA(Q, A, B) {
  const g = K(B, A, !0);
  return !g || g.type !== "comment" && !(g.type === "text" && IB(g.value.charAt(0)));
}
function cE(Q, A, B) {
  const g = K(B, A);
  return !g || g.type !== "comment";
}
function nE(Q, A, B) {
  const g = K(B, A);
  return !g || g.type !== "comment";
}
function aE(Q, A, B) {
  const g = K(B, A);
  return g ? g.type === "element" && (g.tagName === "address" || g.tagName === "article" || g.tagName === "aside" || g.tagName === "blockquote" || g.tagName === "details" || g.tagName === "div" || g.tagName === "dl" || g.tagName === "fieldset" || g.tagName === "figcaption" || g.tagName === "figure" || g.tagName === "footer" || g.tagName === "form" || g.tagName === "h1" || g.tagName === "h2" || g.tagName === "h3" || g.tagName === "h4" || g.tagName === "h5" || g.tagName === "h6" || g.tagName === "header" || g.tagName === "hgroup" || g.tagName === "hr" || g.tagName === "main" || g.tagName === "menu" || g.tagName === "nav" || g.tagName === "ol" || g.tagName === "p" || g.tagName === "pre" || g.tagName === "section" || g.tagName === "table" || g.tagName === "ul") : !B || // Confusing parent.
  !(B.type === "element" && (B.tagName === "a" || B.tagName === "audio" || B.tagName === "del" || B.tagName === "ins" || B.tagName === "map" || B.tagName === "noscript" || B.tagName === "video"));
}
function ME(Q, A, B) {
  const g = K(B, A);
  return !g || g.type === "element" && g.tagName === "li";
}
function hE(Q, A, B) {
  const g = K(B, A);
  return !!(g && g.type === "element" && (g.tagName === "dt" || g.tagName === "dd"));
}
function FE(Q, A, B) {
  const g = K(B, A);
  return !g || g.type === "element" && (g.tagName === "dt" || g.tagName === "dd");
}
function PB(Q, A, B) {
  const g = K(B, A);
  return !g || g.type === "element" && (g.tagName === "rp" || g.tagName === "rt");
}
function GE(Q, A, B) {
  const g = K(B, A);
  return !g || g.type === "element" && g.tagName === "optgroup";
}
function NE(Q, A, B) {
  const g = K(B, A);
  return !g || g.type === "element" && (g.tagName === "option" || g.tagName === "optgroup");
}
function rE(Q, A, B) {
  const g = K(B, A);
  return !!(g && g.type === "element" && (g.tagName === "tbody" || g.tagName === "tfoot"));
}
function LE(Q, A, B) {
  const g = K(B, A);
  return !g || g.type === "element" && (g.tagName === "tbody" || g.tagName === "tfoot");
}
function HE(Q, A, B) {
  return !K(B, A);
}
function RE(Q, A, B) {
  const g = K(B, A);
  return !g || g.type === "element" && g.tagName === "tr";
}
function uB(Q, A, B) {
  const g = K(B, A);
  return !g || g.type === "element" && (g.tagName === "td" || g.tagName === "th");
}
const kE = aQ({
  body: UE,
  colgroup: YE,
  head: KE,
  html: lE,
  tbody: PE
});
function lE(Q) {
  const A = K(Q, -1);
  return !A || A.type !== "comment";
}
function KE(Q) {
  const A = /* @__PURE__ */ new Set();
  for (const g of Q.children)
    if (g.type === "element" && (g.tagName === "base" || g.tagName === "title")) {
      if (A.has(g.tagName)) return !1;
      A.add(g.tagName);
    }
  const B = Q.children[0];
  return !B || B.type === "element";
}
function UE(Q) {
  const A = K(Q, -1, !0);
  return !A || A.type !== "comment" && !(A.type === "text" && IB(A.value.charAt(0))) && !(A.type === "element" && (A.tagName === "meta" || A.tagName === "link" || A.tagName === "script" || A.tagName === "style" || A.tagName === "template"));
}
function YE(Q, A, B) {
  const g = cQ(B, A), E = K(Q, -1, !0);
  return B && g && g.type === "element" && g.tagName === "colgroup" && DB(g, B.children.indexOf(g), B) ? !1 : !!(E && E.type === "element" && E.tagName === "col");
}
function PE(Q, A, B) {
  const g = cQ(B, A), E = K(Q, -1);
  return B && g && g.type === "element" && (g.tagName === "thead" || g.tagName === "tbody") && DB(g, B.children.indexOf(g), B) ? !1 : !!(E && E.type === "element" && E.tagName === "tr");
}
const nA = {
  // See: <https://html.spec.whatwg.org/#attribute-name-state>.
  name: [
    [`	
\f\r &/=>`.split(""), `	
\f\r "&'/=>\``.split("")],
    [`\0	
\f\r "&'/<=>`.split(""), `\0	
\f\r "&'/<=>\``.split("")]
  ],
  // See: <https://html.spec.whatwg.org/#attribute-value-(unquoted)-state>.
  unquoted: [
    [`	
\f\r &>`.split(""), `\0	
\f\r "&'<=>\``.split("")],
    [`\0	
\f\r "&'<=>\``.split(""), `\0	
\f\r "&'<=>\``.split("")]
  ],
  // See: <https://html.spec.whatwg.org/#attribute-value-(single-quoted)-state>.
  single: [
    ["&'".split(""), "\"&'`".split("")],
    ["\0&'".split(""), "\0\"&'`".split("")]
  ],
  // See: <https://html.spec.whatwg.org/#attribute-value-(double-quoted)-state>.
  double: [
    ['"&'.split(""), "\"&'`".split("")],
    ['\0"&'.split(""), "\0\"&'`".split("")]
  ]
};
function uE(Q, A, B, g) {
  const E = g.schema, C = E.space === "svg" ? !1 : g.settings.omitOptionalTags;
  let I = E.space === "svg" ? g.settings.closeEmptyElements : g.settings.voids.includes(Q.tagName.toLowerCase());
  const w = [];
  let D;
  E.space === "html" && Q.tagName === "svg" && (g.schema = iQ);
  const o = yE(g, Q.properties), s = g.all(
    E.space === "html" && Q.tagName === "template" ? Q.content : Q
  );
  return g.schema = E, s && (I = !1), (o || !C || !kE(Q, A, B)) && (w.push("<", Q.tagName, o ? " " + o : ""), I && (E.space === "svg" || g.settings.closeSelfClosing) && (D = o.charAt(o.length - 1), (!g.settings.tightSelfClosing || D === "/" || D && D !== '"' && D !== "'") && w.push(" "), w.push("/")), w.push(">")), w.push(s), !I && (!C || !DB(Q, A, B)) && w.push("</" + Q.tagName + ">"), w.join("");
}
function yE(Q, A) {
  const B = [];
  let g = -1, E;
  if (A) {
    for (E in A)
      if (A[E] !== null && A[E] !== void 0) {
        const C = JE(Q, E, A[E]);
        C && B.push(C);
      }
  }
  for (; ++g < B.length; ) {
    const C = Q.settings.tightAttributes ? B[g].charAt(B[g].length - 1) : void 0;
    g !== B.length - 1 && C !== '"' && C !== "'" && (B[g] += " ");
  }
  return B.join("");
}
function JE(Q, A, B) {
  const g = fg(Q.schema, A), E = Q.settings.allowParseErrors && Q.schema.space === "html" ? 0 : 1, C = Q.settings.allowDangerousCharacters ? 0 : 1;
  let I = Q.quote, w;
  if (g.overloadedBoolean && (B === g.attribute || B === "") ? B = !0 : (g.boolean || g.overloadedBoolean) && (typeof B != "string" || B === g.attribute || B === "") && (B = !!B), B == null || B === !1 || typeof B == "number" && Number.isNaN(B))
    return "";
  const D = j(
    g.attribute,
    Object.assign({}, Q.settings.characterReferences, {
      // Always encode without parse errors in non-HTML.
      subset: nA.name[E][C]
    })
  );
  return B === !0 || (B = Array.isArray(B) ? (g.commaSeparated ? oE : sE)(B, {
    padLeft: !Q.settings.tightCommaSeparatedLists
  }) : String(B), Q.settings.collapseEmptyAttributes && !B) ? D : (Q.settings.preferUnquoted && (w = j(
    B,
    Object.assign({}, Q.settings.characterReferences, {
      attribute: !0,
      subset: nA.unquoted[E][C]
    })
  )), w !== B && (Q.settings.quoteSmart && UB(B, I) > UB(B, Q.alternative) && (I = Q.alternative), w = I + j(
    B,
    Object.assign({}, Q.settings.characterReferences, {
      // Always encode without parse errors in non-HTML.
      subset: (I === "'" ? nA.single : nA.double)[E][C],
      attribute: !0
    })
  ) + I), D + (w && "=" + w));
}
const SE = ["<", "&"];
function MQ(Q, A, B, g) {
  return B && B.type === "element" && (B.tagName === "script" || B.tagName === "style") ? Q.value : j(
    Q.value,
    Object.assign({}, g.settings.characterReferences, {
      subset: SE
    })
  );
}
function pE(Q, A, B, g) {
  return g.settings.allowDangerousHtml ? Q.value : MQ(Q, A, B, g);
}
function fE(Q, A, B, g) {
  return g.all(Q);
}
const dE = mg("type", {
  invalid: OE,
  unknown: xE,
  handlers: { comment: DE, doctype: wE, element: uE, raw: pE, root: fE, text: MQ }
});
function OE(Q) {
  throw new Error("Expected node, not `" + Q + "`");
}
function xE(Q) {
  const A = (
    /** @type {Nodes} */
    Q
  );
  throw new Error("Cannot compile unknown node `" + A.type + "`");
}
const mE = {}, qE = {}, bE = [];
function TE(Q, A) {
  const B = A || mE, g = B.quote || '"', E = g === '"' ? "'" : '"';
  if (g !== '"' && g !== "'")
    throw new Error("Invalid quote `" + g + "`, expected `'` or `\"`");
  return {
    one: WE,
    all: vE,
    settings: {
      omitOptionalTags: B.omitOptionalTags || !1,
      allowParseErrors: B.allowParseErrors || !1,
      allowDangerousCharacters: B.allowDangerousCharacters || !1,
      quoteSmart: B.quoteSmart || !1,
      preferUnquoted: B.preferUnquoted || !1,
      tightAttributes: B.tightAttributes || !1,
      upperDoctype: B.upperDoctype || !1,
      tightDoctype: B.tightDoctype || !1,
      bogusComments: B.bogusComments || !1,
      tightCommaSeparatedLists: B.tightCommaSeparatedLists || !1,
      tightSelfClosing: B.tightSelfClosing || !1,
      collapseEmptyAttributes: B.collapseEmptyAttributes || !1,
      allowDangerousHtml: B.allowDangerousHtml || !1,
      voids: B.voids || Pg,
      characterReferences: B.characterReferences || qE,
      closeSelfClosing: B.closeSelfClosing || !1,
      closeEmptyElements: B.closeEmptyElements || !1
    },
    schema: B.space === "svg" ? iQ : xg,
    quote: g,
    alternative: E
  }.one(
    Array.isArray(Q) ? { type: "root", children: Q } : Q,
    void 0,
    void 0
  );
}
function WE(Q, A, B) {
  return dE(Q, A, B, this);
}
function vE(Q) {
  const A = [], B = Q && Q.children || bE;
  let g = -1;
  for (; ++g < B.length; )
    A[g] = this.one(B[g], g, Q);
  return A.join("");
}
function HA(Q, A) {
  const B = typeof Q == "string" ? {} : { ...Q.colorReplacements }, g = typeof Q == "string" ? Q : Q.name;
  for (const [E, C] of Object.entries((A == null ? void 0 : A.colorReplacements) || {}))
    typeof C == "string" ? B[E] = C : E === g && Object.assign(B, C);
  return B;
}
function T(Q, A) {
  return Q && ((A == null ? void 0 : A[Q == null ? void 0 : Q.toLowerCase()]) || Q);
}
function XE(Q) {
  return Array.isArray(Q) ? Q : [Q];
}
async function hQ(Q) {
  return Promise.resolve(typeof Q == "function" ? Q() : Q).then((A) => A.default || A);
}
function wB(Q) {
  return !Q || ["plaintext", "txt", "text", "plain"].includes(Q);
}
function ZE(Q) {
  return Q === "ansi" || wB(Q);
}
function oB(Q) {
  return Q === "none";
}
function VE(Q) {
  return oB(Q);
}
function FQ(Q, A) {
  var g;
  if (!A)
    return Q;
  Q.properties || (Q.properties = {}), (g = Q.properties).class || (g.class = []), typeof Q.properties.class == "string" && (Q.properties.class = Q.properties.class.split(/\s+/g)), Array.isArray(Q.properties.class) || (Q.properties.class = []);
  const B = Array.isArray(A) ? A : A.split(/\s+/g);
  for (const E of B)
    E && !Q.properties.class.includes(E) && Q.properties.class.push(E);
  return Q;
}
function PA(Q, A = !1) {
  var C;
  const B = Q.split(/(\r?\n)/g);
  let g = 0;
  const E = [];
  for (let I = 0; I < B.length; I += 2) {
    const w = A ? B[I] + (B[I + 1] || "") : B[I];
    E.push([w, g]), g += B[I].length, g += ((C = B[I + 1]) == null ? void 0 : C.length) || 0;
  }
  return E;
}
function zE(Q) {
  const A = PA(Q, !0).map(([E]) => E);
  function B(E) {
    if (E === Q.length)
      return {
        line: A.length - 1,
        character: A[A.length - 1].length
      };
    let C = E, I = 0;
    for (const w of A) {
      if (C < w.length)
        break;
      C -= w.length, I++;
    }
    return { line: I, character: C };
  }
  function g(E, C) {
    let I = 0;
    for (let w = 0; w < E; w++)
      I += A[w].length;
    return I += C, I;
  }
  return {
    lines: A,
    indexToPos: B,
    posToIndex: g
  };
}
const sB = "light-dark()", jE = ["color", "background-color"];
function _E(Q, A) {
  let B = 0;
  const g = [];
  for (const E of A)
    E > B && g.push({
      ...Q,
      content: Q.content.slice(B, E),
      offset: Q.offset + B
    }), B = E;
  return B < Q.content.length && g.push({
    ...Q,
    content: Q.content.slice(B),
    offset: Q.offset + B
  }), g;
}
function $E(Q, A) {
  const B = Array.from(A instanceof Set ? A : new Set(A)).sort((g, E) => g - E);
  return B.length ? Q.map((g) => g.flatMap((E) => {
    const C = B.filter((I) => E.offset < I && I < E.offset + E.content.length).map((I) => I - E.offset).sort((I, w) => I - w);
    return C.length ? _E(E, C) : E;
  })) : Q;
}
function AC(Q, A, B, g, E = "css-vars") {
  const C = {
    content: Q.content,
    explanation: Q.explanation,
    offset: Q.offset
  }, I = A.map((s) => RA(Q.variants[s])), w = new Set(I.flatMap((s) => Object.keys(s))), D = {}, o = (s, e) => {
    const i = e === "color" ? "" : e === "background-color" ? "-bg" : `-${e}`;
    return B + A[s] + (e === "color" ? "" : i);
  };
  return I.forEach((s, e) => {
    for (const i of w) {
      const t = s[i] || "inherit";
      if (e === 0 && g && jE.includes(i))
        if (g === sB && I.length > 1) {
          const n = A.findIndex((a) => a === "light"), N = A.findIndex((a) => a === "dark");
          if (n === -1 || N === -1)
            throw new P('When using `defaultColor: "light-dark()"`, you must provide both `light` and `dark` themes');
          const G = I[n][i] || "inherit", h = I[N][i] || "inherit";
          D[i] = `light-dark(${G}, ${h})`, E === "css-vars" && (D[o(e, i)] = t);
        } else
          D[i] = t;
      else
        E === "css-vars" && (D[o(e, i)] = t);
    }
  }), C.htmlStyle = D, C;
}
function RA(Q) {
  const A = {};
  if (Q.color && (A.color = Q.color), Q.bgColor && (A["background-color"] = Q.bgColor), Q.fontStyle) {
    Q.fontStyle & y.Italic && (A["font-style"] = "italic"), Q.fontStyle & y.Bold && (A["font-weight"] = "bold");
    const B = [];
    Q.fontStyle & y.Underline && B.push("underline"), Q.fontStyle & y.Strikethrough && B.push("line-through"), B.length && (A["text-decoration"] = B.join(" "));
  }
  return A;
}
function QB(Q) {
  return typeof Q == "string" ? Q : Object.entries(Q).map(([A, B]) => `${A}:${B}`).join(";");
}
const GQ = /* @__PURE__ */ new WeakMap();
function uA(Q, A) {
  GQ.set(Q, A);
}
function oA(Q) {
  return GQ.get(Q);
}
class BA {
  constructor(...A) {
    /**
     * Theme to Stack mapping
     */
    c(this, "_stacks", {});
    c(this, "lang");
    if (A.length === 2) {
      const [B, g] = A;
      this.lang = g, this._stacks = B;
    } else {
      const [B, g, E] = A;
      this.lang = g, this._stacks = { [E]: B };
    }
  }
  get themes() {
    return Object.keys(this._stacks);
  }
  get theme() {
    return this.themes[0];
  }
  get _stack() {
    return this._stacks[this.theme];
  }
  /**
   * Static method to create a initial grammar state.
   */
  static initial(A, B) {
    return new BA(
      Object.fromEntries(XE(B).map((g) => [g, jA])),
      A
    );
  }
  /**
   * Get the internal stack object.
   * @internal
   */
  getInternalStack(A = this.theme) {
    return this._stacks[A];
  }
  getScopes(A = this.theme) {
    return BC(this._stacks[A]);
  }
  toJSON() {
    return {
      lang: this.lang,
      theme: this.theme,
      themes: this.themes,
      scopes: this.getScopes()
    };
  }
}
function BC(Q) {
  const A = [], B = /* @__PURE__ */ new Set();
  function g(E) {
    var I;
    if (B.has(E))
      return;
    B.add(E);
    const C = (I = E == null ? void 0 : E.nameScopesList) == null ? void 0 : I.scopeName;
    C && A.push(C), E.parent && g(E.parent);
  }
  return g(Q), A;
}
function QC(Q, A) {
  if (!(Q instanceof BA))
    throw new P("Invalid grammar state");
  return Q.getInternalStack(A);
}
function gC() {
  const Q = /* @__PURE__ */ new WeakMap();
  function A(B) {
    if (!Q.has(B.meta)) {
      let g = function(I) {
        if (typeof I == "number") {
          if (I < 0 || I > B.source.length)
            throw new P(`Invalid decoration offset: ${I}. Code length: ${B.source.length}`);
          return {
            ...E.indexToPos(I),
            offset: I
          };
        } else {
          const w = E.lines[I.line];
          if (w === void 0)
            throw new P(`Invalid decoration position ${JSON.stringify(I)}. Lines length: ${E.lines.length}`);
          let D = I.character;
          if (D < 0 && (D = w.length + D), D < 0 || D > w.length)
            throw new P(`Invalid decoration position ${JSON.stringify(I)}. Line ${I.line} length: ${w.length}`);
          return {
            ...I,
            character: D,
            offset: E.posToIndex(I.line, D)
          };
        }
      };
      const E = zE(B.source), C = (B.options.decorations || []).map((I) => ({
        ...I,
        start: g(I.start),
        end: g(I.end)
      }));
      EC(C), Q.set(B.meta, {
        decorations: C,
        converter: E,
        source: B.source
      });
    }
    return Q.get(B.meta);
  }
  return {
    name: "shiki:decorations",
    tokens(B) {
      var I;
      if (!((I = this.options.decorations) != null && I.length))
        return;
      const E = A(this).decorations.flatMap((w) => [w.start.offset, w.end.offset]);
      return $E(B, E);
    },
    code(B) {
      var s;
      if (!((s = this.options.decorations) != null && s.length))
        return;
      const g = A(this), E = Array.from(B.children).filter((e) => e.type === "element" && e.tagName === "span");
      if (E.length !== g.converter.lines.length)
        throw new P(`Number of lines in code element (${E.length}) does not match the number of lines in the source (${g.converter.lines.length}). Failed to apply decorations.`);
      function C(e, i, t, n) {
        const N = E[e];
        let G = "", h = -1, a = -1;
        if (i === 0 && (h = 0), t === 0 && (a = 0), t === Number.POSITIVE_INFINITY && (a = N.children.length), h === -1 || a === -1)
          for (let L = 0; L < N.children.length; L++)
            G += NQ(N.children[L]), h === -1 && G.length === i && (h = L + 1), a === -1 && G.length === t && (a = L + 1);
        if (h === -1)
          throw new P(`Failed to find start index for decoration ${JSON.stringify(n.start)}`);
        if (a === -1)
          throw new P(`Failed to find end index for decoration ${JSON.stringify(n.end)}`);
        const F = N.children.slice(h, a);
        if (!n.alwaysWrap && F.length === N.children.length)
          w(N, n, "line");
        else if (!n.alwaysWrap && F.length === 1 && F[0].type === "element")
          w(F[0], n, "token");
        else {
          const L = {
            type: "element",
            tagName: "span",
            properties: {},
            children: F
          };
          w(L, n, "wrapper"), N.children.splice(h, F.length, L);
        }
      }
      function I(e, i) {
        E[e] = w(E[e], i, "line");
      }
      function w(e, i, t) {
        var G;
        const n = i.properties || {}, N = i.transform || ((h) => h);
        return e.tagName = i.tagName || "span", e.properties = {
          ...e.properties,
          ...n,
          class: e.properties.class
        }, (G = i.properties) != null && G.class && FQ(e, i.properties.class), e = N(e, t) || e, e;
      }
      const D = [], o = g.decorations.sort((e, i) => i.start.offset - e.start.offset || e.end.offset - i.end.offset);
      for (const e of o) {
        const { start: i, end: t } = e;
        if (i.line === t.line)
          C(i.line, i.character, t.character, e);
        else if (i.line < t.line) {
          C(i.line, i.character, Number.POSITIVE_INFINITY, e);
          for (let n = i.line + 1; n < t.line; n++)
            D.unshift(() => I(n, e));
          C(t.line, 0, t.character, e);
        }
      }
      D.forEach((e) => e());
    }
  };
}
function EC(Q) {
  for (let A = 0; A < Q.length; A++) {
    const B = Q[A];
    if (B.start.offset > B.end.offset)
      throw new P(`Invalid decoration range: ${JSON.stringify(B.start)} - ${JSON.stringify(B.end)}`);
    for (let g = A + 1; g < Q.length; g++) {
      const E = Q[g], C = B.start.offset <= E.start.offset && E.start.offset < B.end.offset, I = B.start.offset < E.end.offset && E.end.offset <= B.end.offset, w = E.start.offset <= B.start.offset && B.start.offset < E.end.offset, D = E.start.offset < B.end.offset && B.end.offset <= E.end.offset;
      if (C || I || w || D) {
        if (C && I || w && D || w && B.start.offset === B.end.offset || I && E.start.offset === E.end.offset)
          continue;
        throw new P(`Decorations ${JSON.stringify(B.start)} and ${JSON.stringify(E.start)} intersect.`);
      }
    }
  }
}
function NQ(Q) {
  return Q.type === "text" ? Q.value : Q.type === "element" ? Q.children.map(NQ).join("") : "";
}
const CC = [
  /* @__PURE__ */ gC()
];
function kA(Q) {
  const A = IC(Q.transformers || []);
  return [
    ...A.pre,
    ...A.normal,
    ...A.post,
    ...CC
  ];
}
function IC(Q) {
  const A = [], B = [], g = [];
  for (const E of Q)
    switch (E.enforce) {
      case "pre":
        A.push(E);
        break;
      case "post":
        B.push(E);
        break;
      default:
        g.push(E);
    }
  return { pre: A, post: B, normal: g };
}
var W = [
  "black",
  "red",
  "green",
  "yellow",
  "blue",
  "magenta",
  "cyan",
  "white",
  "brightBlack",
  "brightRed",
  "brightGreen",
  "brightYellow",
  "brightBlue",
  "brightMagenta",
  "brightCyan",
  "brightWhite"
], TA = {
  1: "bold",
  2: "dim",
  3: "italic",
  4: "underline",
  7: "reverse",
  8: "hidden",
  9: "strikethrough"
};
function DC(Q, A) {
  const B = Q.indexOf("\x1B", A);
  if (B !== -1 && Q[B + 1] === "[") {
    const g = Q.indexOf("m", B);
    if (g !== -1)
      return {
        sequence: Q.substring(B + 2, g).split(";"),
        startPosition: B,
        position: g + 1
      };
  }
  return {
    position: Q.length
  };
}
function yB(Q) {
  const A = Q.shift();
  if (A === "2") {
    const B = Q.splice(0, 3).map((g) => Number.parseInt(g));
    return B.length !== 3 || B.some((g) => Number.isNaN(g)) ? void 0 : {
      type: "rgb",
      rgb: B
    };
  } else if (A === "5") {
    const B = Q.shift();
    if (B)
      return { type: "table", index: Number(B) };
  }
}
function wC(Q) {
  const A = [];
  for (; Q.length > 0; ) {
    const B = Q.shift();
    if (!B)
      continue;
    const g = Number.parseInt(B);
    if (!Number.isNaN(g))
      if (g === 0)
        A.push({ type: "resetAll" });
      else if (g <= 9)
        TA[g] && A.push({
          type: "setDecoration",
          value: TA[g]
        });
      else if (g <= 29) {
        const E = TA[g - 20];
        E && (A.push({
          type: "resetDecoration",
          value: E
        }), E === "dim" && A.push({
          type: "resetDecoration",
          value: "bold"
        }));
      } else if (g <= 37)
        A.push({
          type: "setForegroundColor",
          value: { type: "named", name: W[g - 30] }
        });
      else if (g === 38) {
        const E = yB(Q);
        E && A.push({
          type: "setForegroundColor",
          value: E
        });
      } else if (g === 39)
        A.push({
          type: "resetForegroundColor"
        });
      else if (g <= 47)
        A.push({
          type: "setBackgroundColor",
          value: { type: "named", name: W[g - 40] }
        });
      else if (g === 48) {
        const E = yB(Q);
        E && A.push({
          type: "setBackgroundColor",
          value: E
        });
      } else g === 49 ? A.push({
        type: "resetBackgroundColor"
      }) : g === 53 ? A.push({
        type: "setDecoration",
        value: "overline"
      }) : g === 55 ? A.push({
        type: "resetDecoration",
        value: "overline"
      }) : g >= 90 && g <= 97 ? A.push({
        type: "setForegroundColor",
        value: { type: "named", name: W[g - 90 + 8] }
      }) : g >= 100 && g <= 107 && A.push({
        type: "setBackgroundColor",
        value: { type: "named", name: W[g - 100 + 8] }
      });
  }
  return A;
}
function oC() {
  let Q = null, A = null, B = /* @__PURE__ */ new Set();
  return {
    parse(g) {
      const E = [];
      let C = 0;
      do {
        const I = DC(g, C), w = I.sequence ? g.substring(C, I.startPosition) : g.substring(C);
        if (w.length > 0 && E.push({
          value: w,
          foreground: Q,
          background: A,
          decorations: new Set(B)
        }), I.sequence) {
          const D = wC(I.sequence);
          for (const o of D)
            o.type === "resetAll" ? (Q = null, A = null, B.clear()) : o.type === "resetForegroundColor" ? Q = null : o.type === "resetBackgroundColor" ? A = null : o.type === "resetDecoration" && B.delete(o.value);
          for (const o of D)
            o.type === "setForegroundColor" ? Q = o.value : o.type === "setBackgroundColor" ? A = o.value : o.type === "setDecoration" && B.add(o.value);
        }
        C = I.position;
      } while (C < g.length);
      return E;
    }
  };
}
var sC = {
  black: "#000000",
  red: "#bb0000",
  green: "#00bb00",
  yellow: "#bbbb00",
  blue: "#0000bb",
  magenta: "#ff00ff",
  cyan: "#00bbbb",
  white: "#eeeeee",
  brightBlack: "#555555",
  brightRed: "#ff5555",
  brightGreen: "#00ff00",
  brightYellow: "#ffff55",
  brightBlue: "#5555ff",
  brightMagenta: "#ff55ff",
  brightCyan: "#55ffff",
  brightWhite: "#ffffff"
};
function eC(Q = sC) {
  function A(w) {
    return Q[w];
  }
  function B(w) {
    return `#${w.map((D) => Math.max(0, Math.min(D, 255)).toString(16).padStart(2, "0")).join("")}`;
  }
  let g;
  function E() {
    if (g)
      return g;
    g = [];
    for (let o = 0; o < W.length; o++)
      g.push(A(W[o]));
    let w = [0, 95, 135, 175, 215, 255];
    for (let o = 0; o < 6; o++)
      for (let s = 0; s < 6; s++)
        for (let e = 0; e < 6; e++)
          g.push(B([w[o], w[s], w[e]]));
    let D = 8;
    for (let o = 0; o < 24; o++, D += 10)
      g.push(B([D, D, D]));
    return g;
  }
  function C(w) {
    return E()[w];
  }
  function I(w) {
    switch (w.type) {
      case "named":
        return A(w.name);
      case "rgb":
        return B(w.rgb);
      case "table":
        return C(w.index);
    }
  }
  return {
    value: I
  };
}
function iC(Q, A, B) {
  const g = HA(Q, B), E = PA(A), C = eC(
    Object.fromEntries(
      W.map((w) => {
        var D;
        return [
          w,
          (D = Q.colors) == null ? void 0 : D[`terminal.ansi${w[0].toUpperCase()}${w.substring(1)}`]
        ];
      })
    )
  ), I = oC();
  return E.map(
    (w) => I.parse(w[0]).map((D) => {
      let o, s;
      D.decorations.has("reverse") ? (o = D.background ? C.value(D.background) : Q.bg, s = D.foreground ? C.value(D.foreground) : Q.fg) : (o = D.foreground ? C.value(D.foreground) : Q.fg, s = D.background ? C.value(D.background) : void 0), o = T(o, g), s = T(s, g), D.decorations.has("dim") && (o = tC(o));
      let e = y.None;
      return D.decorations.has("bold") && (e |= y.Bold), D.decorations.has("italic") && (e |= y.Italic), D.decorations.has("underline") && (e |= y.Underline), D.decorations.has("strikethrough") && (e |= y.Strikethrough), {
        content: D.value,
        offset: w[1],
        // TODO: more accurate offset? might need to fork ansi-sequence-parser
        color: o,
        bgColor: s,
        fontStyle: e
      };
    })
  );
}
function tC(Q) {
  const A = Q.match(/#([0-9a-f]{3})([0-9a-f]{3})?([0-9a-f]{2})?/);
  if (A)
    if (A[3]) {
      const g = Math.round(Number.parseInt(A[3], 16) / 2).toString(16).padStart(2, "0");
      return `#${A[1]}${A[2]}${g}`;
    } else return A[2] ? `#${A[1]}${A[2]}80` : `#${Array.from(A[1]).map((g) => `${g}${g}`).join("")}80`;
  const B = Q.match(/var\((--[\w-]+-ansi-[\w-]+)\)/);
  return B ? `var(${B[1]}-dim)` : Q;
}
function eB(Q, A, B = {}) {
  const {
    lang: g = "text",
    theme: E = Q.getLoadedThemes()[0]
  } = B;
  if (wB(g) || oB(E))
    return PA(A).map((D) => [{ content: D[0], offset: D[1] }]);
  const { theme: C, colorMap: I } = Q.setTheme(E);
  if (g === "ansi")
    return iC(C, A, B);
  const w = Q.getLanguage(g);
  if (B.grammarState) {
    if (B.grammarState.lang !== w.name)
      throw new P(`Grammar state language "${B.grammarState.lang}" does not match highlight language "${w.name}"`);
    if (!B.grammarState.themes.includes(C.name))
      throw new P(`Grammar state themes "${B.grammarState.themes}" do not contain highlight theme "${C.name}"`);
  }
  return nC(A, w, C, I, B);
}
function cC(...Q) {
  if (Q.length === 2)
    return oA(Q[1]);
  const [A, B, g = {}] = Q, {
    lang: E = "text",
    theme: C = A.getLoadedThemes()[0]
  } = g;
  if (wB(E) || oB(C))
    throw new P("Plain language does not have grammar state");
  if (E === "ansi")
    throw new P("ANSI language does not have grammar state");
  const { theme: I, colorMap: w } = A.setTheme(C), D = A.getLanguage(E);
  return new BA(
    lA(B, D, I, w, g).stateStack,
    D.name,
    I.name
  );
}
function nC(Q, A, B, g, E) {
  const C = lA(Q, A, B, g, E), I = new BA(
    lA(Q, A, B, g, E).stateStack,
    A.name,
    B.name
  );
  return uA(C.tokens, I), C.tokens;
}
function lA(Q, A, B, g, E) {
  const C = HA(B, E), {
    tokenizeMaxLineLength: I = 0,
    tokenizeTimeLimit: w = 500
  } = E, D = PA(Q);
  let o = E.grammarState ? QC(E.grammarState, B.name) ?? jA : E.grammarContextCode != null ? lA(
    E.grammarContextCode,
    A,
    B,
    g,
    {
      ...E,
      grammarState: void 0,
      grammarContextCode: void 0
    }
  ).stateStack : jA, s = [];
  const e = [];
  for (let i = 0, t = D.length; i < t; i++) {
    const [n, N] = D[i];
    if (n === "") {
      s = [], e.push([]);
      continue;
    }
    if (I > 0 && n.length >= I) {
      s = [], e.push([{
        content: n,
        offset: N,
        color: "",
        fontStyle: 0
      }]);
      continue;
    }
    let G, h, a;
    E.includeExplanation && (G = A.tokenizeLine(n, o, w), h = G.tokens, a = 0);
    const F = A.tokenizeLine2(n, o, w), L = F.tokens.length / 2;
    for (let R = 0; R < L; R++) {
      const J = F.tokens[2 * R], U = R + 1 < L ? F.tokens[2 * R + 2] : n.length;
      if (J === U)
        continue;
      const m = F.tokens[2 * R + 1], iA = T(
        g[$.getForeground(m)],
        C
      ), QA = $.getFontStyle(m), SA = {
        content: n.substring(J, U),
        offset: N + J,
        color: iA,
        fontStyle: QA
      };
      if (E.includeExplanation) {
        const cB = [];
        if (E.includeExplanation !== "scopeName")
          for (const q of B.settings) {
            let X;
            switch (typeof q.scope) {
              case "string":
                X = q.scope.split(/,/).map((pA) => pA.trim());
                break;
              case "object":
                X = q.scope;
                break;
              default:
                continue;
            }
            cB.push({
              settings: q,
              selectors: X.map((pA) => pA.split(/ /))
            });
          }
        SA.explanation = [];
        let nB = 0;
        for (; J + nB < U; ) {
          const q = h[a], X = n.substring(
            q.startIndex,
            q.endIndex
          );
          nB += X.length, SA.explanation.push({
            content: X,
            scopes: E.includeExplanation === "scopeName" ? aC(
              q.scopes
            ) : MC(
              cB,
              q.scopes
            )
          }), a += 1;
        }
      }
      s.push(SA);
    }
    e.push(s), s = [], o = F.ruleStack;
  }
  return {
    tokens: e,
    stateStack: o
  };
}
function aC(Q) {
  return Q.map((A) => ({ scopeName: A }));
}
function MC(Q, A) {
  const B = [];
  for (let g = 0, E = A.length; g < E; g++) {
    const C = A[g];
    B[g] = {
      scopeName: C,
      themeMatches: FC(Q, C, A.slice(0, g))
    };
  }
  return B;
}
function JB(Q, A) {
  return Q === A || A.substring(0, Q.length) === Q && A[Q.length] === ".";
}
function hC(Q, A, B) {
  if (!JB(Q[Q.length - 1], A))
    return !1;
  let g = Q.length - 2, E = B.length - 1;
  for (; g >= 0 && E >= 0; )
    JB(Q[g], B[E]) && (g -= 1), E -= 1;
  return g === -1;
}
function FC(Q, A, B) {
  const g = [];
  for (const { selectors: E, settings: C } of Q)
    for (const I of E)
      if (hC(I, A, B)) {
        g.push(C);
        break;
      }
  return g;
}
function rQ(Q, A, B) {
  const g = Object.entries(B.themes).filter((D) => D[1]).map((D) => ({ color: D[0], theme: D[1] })), E = g.map((D) => {
    const o = eB(Q, A, {
      ...B,
      theme: D.theme
    }), s = oA(o), e = typeof D.theme == "string" ? D.theme : D.theme.name;
    return {
      tokens: o,
      state: s,
      theme: e
    };
  }), C = GC(
    ...E.map((D) => D.tokens)
  ), I = C[0].map(
    (D, o) => D.map((s, e) => {
      const i = {
        content: s.content,
        variants: {},
        offset: s.offset
      };
      return "includeExplanation" in B && B.includeExplanation && (i.explanation = s.explanation), C.forEach((t, n) => {
        const {
          content: N,
          explanation: G,
          offset: h,
          ...a
        } = t[o][e];
        i.variants[g[n].color] = a;
      }), i;
    })
  ), w = E[0].state ? new BA(
    Object.fromEntries(E.map((D) => {
      var o;
      return [D.theme, (o = D.state) == null ? void 0 : o.getInternalStack(D.theme)];
    })),
    E[0].state.lang
  ) : void 0;
  return w && uA(I, w), I;
}
function GC(...Q) {
  const A = Q.map(() => []), B = Q.length;
  for (let g = 0; g < Q[0].length; g++) {
    const E = Q.map((D) => D[g]), C = A.map(() => []);
    A.forEach((D, o) => D.push(C[o]));
    const I = E.map(() => 0), w = E.map((D) => D[0]);
    for (; w.every((D) => D); ) {
      const D = Math.min(...w.map((o) => o.content.length));
      for (let o = 0; o < B; o++) {
        const s = w[o];
        s.content.length === D ? (C[o].push(s), I[o] += 1, w[o] = E[o][I[o]]) : (C[o].push({
          ...s,
          content: s.content.slice(0, D)
        }), w[o] = {
          ...s,
          content: s.content.slice(D),
          offset: s.offset + D
        });
      }
    }
  }
  return A;
}
function KA(Q, A, B) {
  let g, E, C, I, w, D;
  if ("themes" in B) {
    const {
      defaultColor: o = "light",
      cssVariablePrefix: s = "--shiki-",
      colorsRendering: e = "css-vars"
    } = B, i = Object.entries(B.themes).filter((h) => h[1]).map((h) => ({ color: h[0], theme: h[1] })).sort((h, a) => h.color === o ? -1 : a.color === o ? 1 : 0);
    if (i.length === 0)
      throw new P("`themes` option must not be empty");
    const t = rQ(
      Q,
      A,
      B
    );
    if (D = oA(t), o && sB !== o && !i.find((h) => h.color === o))
      throw new P(`\`themes\` option must contain the defaultColor key \`${o}\``);
    const n = i.map((h) => Q.getTheme(h.theme)), N = i.map((h) => h.color);
    C = t.map((h) => h.map((a) => AC(a, N, s, o, e))), D && uA(C, D);
    const G = i.map((h) => HA(h.theme, B));
    E = SB(i, n, G, s, o, "fg", e), g = SB(i, n, G, s, o, "bg", e), I = `shiki-themes ${n.map((h) => h.name).join(" ")}`, w = o ? void 0 : [E, g].join(";");
  } else if ("theme" in B) {
    const o = HA(B.theme, B);
    C = eB(
      Q,
      A,
      B
    );
    const s = Q.getTheme(B.theme);
    g = T(s.bg, o), E = T(s.fg, o), I = s.name, D = oA(C);
  } else
    throw new P("Invalid options, either `theme` or `themes` must be provided");
  return {
    tokens: C,
    fg: E,
    bg: g,
    themeName: I,
    rootStyle: w,
    grammarState: D
  };
}
function SB(Q, A, B, g, E, C, I) {
  return Q.map((w, D) => {
    const o = T(A[D][C], B[D]) || "inherit", s = `${g + w.color}${C === "bg" ? "-bg" : ""}:${o}`;
    if (D === 0 && E) {
      if (E === sB && Q.length > 1) {
        const e = Q.findIndex((N) => N.color === "light"), i = Q.findIndex((N) => N.color === "dark");
        if (e === -1 || i === -1)
          throw new P('When using `defaultColor: "light-dark()"`, you must provide both `light` and `dark` themes');
        const t = T(A[e][C], B[e]) || "inherit", n = T(A[i][C], B[i]) || "inherit";
        return `light-dark(${t}, ${n});${s}`;
      }
      return o;
    }
    return I === "css-vars" ? s : null;
  }).filter((w) => !!w).join(";");
}
function UA(Q, A, B, g = {
  meta: {},
  options: B,
  codeToHast: (E, C) => UA(Q, E, C),
  codeToTokens: (E, C) => KA(Q, E, C)
}) {
  var n, N;
  let E = A;
  for (const G of kA(B))
    E = ((n = G.preprocess) == null ? void 0 : n.call(g, E, B)) || E;
  let {
    tokens: C,
    fg: I,
    bg: w,
    themeName: D,
    rootStyle: o,
    grammarState: s
  } = KA(Q, E, B);
  const {
    mergeWhitespaces: e = !0,
    mergeSameStyleTokens: i = !1
  } = B;
  e === !0 ? C = rC(C) : e === "never" && (C = LC(C)), i && (C = HC(C));
  const t = {
    ...g,
    get source() {
      return E;
    }
  };
  for (const G of kA(B))
    C = ((N = G.tokens) == null ? void 0 : N.call(t, C)) || C;
  return NC(
    C,
    {
      ...B,
      fg: I,
      bg: w,
      themeName: D,
      rootStyle: o
    },
    t,
    s
  );
}
function NC(Q, A, B, g = oA(Q)) {
  var n, N, G;
  const E = kA(A), C = [], I = {
    type: "root",
    children: []
  }, {
    structure: w = "classic",
    tabindex: D = "0"
  } = A;
  let o = {
    type: "element",
    tagName: "pre",
    properties: {
      class: `shiki ${A.themeName || ""}`,
      style: A.rootStyle || `background-color:${A.bg};color:${A.fg}`,
      ...D !== !1 && D != null ? {
        tabindex: D.toString()
      } : {},
      ...Object.fromEntries(
        Array.from(
          Object.entries(A.meta || {})
        ).filter(([h]) => !h.startsWith("_"))
      )
    },
    children: []
  }, s = {
    type: "element",
    tagName: "code",
    properties: {},
    children: C
  };
  const e = [], i = {
    ...B,
    structure: w,
    addClassToHast: FQ,
    get source() {
      return B.source;
    },
    get tokens() {
      return Q;
    },
    get options() {
      return A;
    },
    get root() {
      return I;
    },
    get pre() {
      return o;
    },
    get code() {
      return s;
    },
    get lines() {
      return e;
    }
  };
  if (Q.forEach((h, a) => {
    var R, J;
    a && (w === "inline" ? I.children.push({ type: "element", tagName: "br", properties: {}, children: [] }) : w === "classic" && C.push({ type: "text", value: `
` }));
    let F = {
      type: "element",
      tagName: "span",
      properties: { class: "line" },
      children: []
    }, L = 0;
    for (const U of h) {
      let m = {
        type: "element",
        tagName: "span",
        properties: {
          ...U.htmlAttrs
        },
        children: [{ type: "text", value: U.content }]
      };
      const iA = QB(U.htmlStyle || RA(U));
      iA && (m.properties.style = iA);
      for (const QA of E)
        m = ((R = QA == null ? void 0 : QA.span) == null ? void 0 : R.call(i, m, a + 1, L, F, U)) || m;
      w === "inline" ? I.children.push(m) : w === "classic" && F.children.push(m), L += U.content.length;
    }
    if (w === "classic") {
      for (const U of E)
        F = ((J = U == null ? void 0 : U.line) == null ? void 0 : J.call(i, F, a + 1)) || F;
      e.push(F), C.push(F);
    }
  }), w === "classic") {
    for (const h of E)
      s = ((n = h == null ? void 0 : h.code) == null ? void 0 : n.call(i, s)) || s;
    o.children.push(s);
    for (const h of E)
      o = ((N = h == null ? void 0 : h.pre) == null ? void 0 : N.call(i, o)) || o;
    I.children.push(o);
  }
  let t = I;
  for (const h of E)
    t = ((G = h == null ? void 0 : h.root) == null ? void 0 : G.call(i, t)) || t;
  return g && uA(t, g), t;
}
function rC(Q) {
  return Q.map((A) => {
    const B = [];
    let g = "", E = 0;
    return A.forEach((C, I) => {
      const D = !(C.fontStyle && (C.fontStyle & y.Underline || C.fontStyle & y.Strikethrough));
      D && C.content.match(/^\s+$/) && A[I + 1] ? (E || (E = C.offset), g += C.content) : g ? (D ? B.push({
        ...C,
        offset: E,
        content: g + C.content
      }) : B.push(
        {
          content: g,
          offset: E
        },
        C
      ), E = 0, g = "") : B.push(C);
    }), B;
  });
}
function LC(Q) {
  return Q.map((A) => A.flatMap((B) => {
    if (B.content.match(/^\s+$/))
      return B;
    const g = B.content.match(/^(\s*)(.*?)(\s*)$/);
    if (!g)
      return B;
    const [, E, C, I] = g;
    if (!E && !I)
      return B;
    const w = [{
      ...B,
      offset: B.offset + E.length,
      content: C
    }];
    return E && w.unshift({
      content: E,
      offset: B.offset
    }), I && w.push({
      content: I,
      offset: B.offset + E.length + C.length
    }), w;
  }));
}
function HC(Q) {
  return Q.map((A) => {
    const B = [];
    for (const g of A) {
      if (B.length === 0) {
        B.push({ ...g });
        continue;
      }
      const E = B[B.length - 1], C = QB(E.htmlStyle || RA(E)), I = QB(g.htmlStyle || RA(g)), w = E.fontStyle && (E.fontStyle & y.Underline || E.fontStyle & y.Strikethrough), D = g.fontStyle && (g.fontStyle & y.Underline || g.fontStyle & y.Strikethrough);
      !w && !D && C === I ? E.content += g.content : B.push({ ...g });
    }
    return B;
  });
}
const RC = TE;
function kC(Q, A, B) {
  var C;
  const g = {
    meta: {},
    options: B,
    codeToHast: (I, w) => UA(Q, I, w),
    codeToTokens: (I, w) => KA(Q, I, w)
  };
  let E = RC(UA(Q, A, B, g));
  for (const I of kA(B))
    E = ((C = I.postprocess) == null ? void 0 : C.call(g, E, B)) || E;
  return E;
}
const pB = { light: "#333333", dark: "#bbbbbb" }, fB = { light: "#fffffe", dark: "#1e1e1e" }, dB = "__shiki_resolved";
function iB(Q) {
  var w, D, o, s, e;
  if (Q != null && Q[dB])
    return Q;
  const A = {
    ...Q
  };
  A.tokenColors && !A.settings && (A.settings = A.tokenColors, delete A.tokenColors), A.type || (A.type = "dark"), A.colorReplacements = { ...A.colorReplacements }, A.settings || (A.settings = []);
  let { bg: B, fg: g } = A;
  if (!B || !g) {
    const i = A.settings ? A.settings.find((t) => !t.name && !t.scope) : void 0;
    (w = i == null ? void 0 : i.settings) != null && w.foreground && (g = i.settings.foreground), (D = i == null ? void 0 : i.settings) != null && D.background && (B = i.settings.background), !g && ((o = A == null ? void 0 : A.colors) != null && o["editor.foreground"]) && (g = A.colors["editor.foreground"]), !B && ((s = A == null ? void 0 : A.colors) != null && s["editor.background"]) && (B = A.colors["editor.background"]), g || (g = A.type === "light" ? pB.light : pB.dark), B || (B = A.type === "light" ? fB.light : fB.dark), A.fg = g, A.bg = B;
  }
  A.settings[0] && A.settings[0].settings && !A.settings[0].scope || A.settings.unshift({
    settings: {
      foreground: A.fg,
      background: A.bg
    }
  });
  let E = 0;
  const C = /* @__PURE__ */ new Map();
  function I(i) {
    var n;
    if (C.has(i))
      return C.get(i);
    E += 1;
    const t = `#${E.toString(16).padStart(8, "0").toLowerCase()}`;
    return (n = A.colorReplacements) != null && n[`#${t}`] ? I(i) : (C.set(i, t), t);
  }
  A.settings = A.settings.map((i) => {
    var G, h;
    const t = ((G = i.settings) == null ? void 0 : G.foreground) && !i.settings.foreground.startsWith("#"), n = ((h = i.settings) == null ? void 0 : h.background) && !i.settings.background.startsWith("#");
    if (!t && !n)
      return i;
    const N = {
      ...i,
      settings: {
        ...i.settings
      }
    };
    if (t) {
      const a = I(i.settings.foreground);
      A.colorReplacements[a] = i.settings.foreground, N.settings.foreground = a;
    }
    if (n) {
      const a = I(i.settings.background);
      A.colorReplacements[a] = i.settings.background, N.settings.background = a;
    }
    return N;
  });
  for (const i of Object.keys(A.colors || {}))
    if ((i === "editor.foreground" || i === "editor.background" || i.startsWith("terminal.ansi")) && !((e = A.colors[i]) != null && e.startsWith("#"))) {
      const t = I(A.colors[i]);
      A.colorReplacements[t] = A.colors[i], A.colors[i] = t;
    }
  return Object.defineProperty(A, dB, {
    enumerable: !1,
    writable: !1,
    value: !0
  }), A;
}
async function LQ(Q) {
  return Array.from(new Set((await Promise.all(
    Q.filter((A) => !ZE(A)).map(async (A) => await hQ(A).then((B) => Array.isArray(B) ? B : [B]))
  )).flat()));
}
async function HQ(Q) {
  return (await Promise.all(
    Q.map(
      async (B) => VE(B) ? null : iB(await hQ(B))
    )
  )).filter((B) => !!B);
}
let lC = 3;
function KC(Q, A = 3) {
  A > lC || console.trace(`[SHIKI DEPRECATE]: ${Q}`);
}
let Z = class extends Error {
  constructor(A) {
    super(A), this.name = "ShikiError";
  }
};
class UC extends Yg {
  constructor(B, g, E, C = {}) {
    super(B);
    c(this, "_resolvedThemes", /* @__PURE__ */ new Map());
    c(this, "_resolvedGrammars", /* @__PURE__ */ new Map());
    c(this, "_langMap", /* @__PURE__ */ new Map());
    c(this, "_langGraph", /* @__PURE__ */ new Map());
    c(this, "_textmateThemeCache", /* @__PURE__ */ new WeakMap());
    c(this, "_loadedThemesCache", null);
    c(this, "_loadedLanguagesCache", null);
    this._resolver = B, this._themes = g, this._langs = E, this._alias = C, this._themes.map((I) => this.loadTheme(I)), this.loadLanguages(this._langs);
  }
  getTheme(B) {
    return typeof B == "string" ? this._resolvedThemes.get(B) : this.loadTheme(B);
  }
  loadTheme(B) {
    const g = iB(B);
    return g.name && (this._resolvedThemes.set(g.name, g), this._loadedThemesCache = null), g;
  }
  getLoadedThemes() {
    return this._loadedThemesCache || (this._loadedThemesCache = [...this._resolvedThemes.keys()]), this._loadedThemesCache;
  }
  // Override and re-implement this method to cache the textmate themes as `TextMateTheme.createFromRawTheme`
  // is expensive. Themes can switch often especially for dual-theme support.
  //
  // The parent class also accepts `colorMap` as the second parameter, but since we don't use that,
  // we omit here so it's easier to cache the themes.
  setTheme(B) {
    let g = this._textmateThemeCache.get(B);
    g || (g = FA.createFromRawTheme(B), this._textmateThemeCache.set(B, g)), this._syncRegistry.setTheme(g);
  }
  getGrammar(B) {
    if (this._alias[B]) {
      const g = /* @__PURE__ */ new Set([B]);
      for (; this._alias[B]; ) {
        if (B = this._alias[B], g.has(B))
          throw new Z(`Circular alias \`${Array.from(g).join(" -> ")} -> ${B}\``);
        g.add(B);
      }
    }
    return this._resolvedGrammars.get(B);
  }
  loadLanguage(B) {
    var I, w, D, o;
    if (this.getGrammar(B.name))
      return;
    const g = new Set(
      [...this._langMap.values()].filter((s) => {
        var e;
        return (e = s.embeddedLangsLazy) == null ? void 0 : e.includes(B.name);
      })
    );
    this._resolver.addLanguage(B);
    const E = {
      balancedBracketSelectors: B.balancedBracketSelectors || ["*"],
      unbalancedBracketSelectors: B.unbalancedBracketSelectors || []
    };
    this._syncRegistry._rawGrammars.set(B.scopeName, B);
    const C = this.loadGrammarWithConfiguration(B.scopeName, 1, E);
    if (C.name = B.name, this._resolvedGrammars.set(B.name, C), B.aliases && B.aliases.forEach((s) => {
      this._alias[s] = B.name;
    }), this._loadedLanguagesCache = null, g.size)
      for (const s of g)
        this._resolvedGrammars.delete(s.name), this._loadedLanguagesCache = null, (w = (I = this._syncRegistry) == null ? void 0 : I._injectionGrammars) == null || w.delete(s.scopeName), (o = (D = this._syncRegistry) == null ? void 0 : D._grammars) == null || o.delete(s.scopeName), this.loadLanguage(this._langMap.get(s.name));
  }
  dispose() {
    super.dispose(), this._resolvedThemes.clear(), this._resolvedGrammars.clear(), this._langMap.clear(), this._langGraph.clear(), this._loadedThemesCache = null;
  }
  loadLanguages(B) {
    for (const C of B)
      this.resolveEmbeddedLanguages(C);
    const g = Array.from(this._langGraph.entries()), E = g.filter(([C, I]) => !I);
    if (E.length) {
      const C = g.filter(([I, w]) => {
        var D;
        return w && ((D = w.embeddedLangs) == null ? void 0 : D.some((o) => E.map(([s]) => s).includes(o)));
      }).filter((I) => !E.includes(I));
      throw new Z(`Missing languages ${E.map(([I]) => `\`${I}\``).join(", ")}, required by ${C.map(([I]) => `\`${I}\``).join(", ")}`);
    }
    for (const [C, I] of g)
      this._resolver.addLanguage(I);
    for (const [C, I] of g)
      this.loadLanguage(I);
  }
  getLoadedLanguages() {
    return this._loadedLanguagesCache || (this._loadedLanguagesCache = [
      .../* @__PURE__ */ new Set([...this._resolvedGrammars.keys(), ...Object.keys(this._alias)])
    ]), this._loadedLanguagesCache;
  }
  resolveEmbeddedLanguages(B) {
    if (this._langMap.set(B.name, B), this._langGraph.set(B.name, B), B.embeddedLangs)
      for (const g of B.embeddedLangs)
        this._langGraph.set(g, this._langMap.get(g));
  }
}
class YC {
  constructor(A, B) {
    c(this, "_langs", /* @__PURE__ */ new Map());
    c(this, "_scopeToLang", /* @__PURE__ */ new Map());
    c(this, "_injections", /* @__PURE__ */ new Map());
    c(this, "_onigLib");
    this._onigLib = {
      createOnigScanner: (g) => A.createScanner(g),
      createOnigString: (g) => A.createString(g)
    }, B.forEach((g) => this.addLanguage(g));
  }
  get onigLib() {
    return this._onigLib;
  }
  getLangRegistration(A) {
    return this._langs.get(A);
  }
  loadGrammar(A) {
    return this._scopeToLang.get(A);
  }
  addLanguage(A) {
    this._langs.set(A.name, A), A.aliases && A.aliases.forEach((B) => {
      this._langs.set(B, A);
    }), this._scopeToLang.set(A.scopeName, A), A.injectTo && A.injectTo.forEach((B) => {
      this._injections.get(B) || this._injections.set(B, []), this._injections.get(B).push(A.scopeName);
    });
  }
  getInjections(A) {
    const B = A.split(".");
    let g = [];
    for (let E = 1; E <= B.length; E++) {
      const C = B.slice(0, E).join(".");
      g = [...g, ...this._injections.get(C) || []];
    }
    return g;
  }
}
let gA = 0;
function PC(Q) {
  gA += 1, Q.warnings !== !1 && gA >= 10 && gA % 10 === 0 && console.warn(`[Shiki] ${gA} instances have been created. Shiki is supposed to be used as a singleton, consider refactoring your code to cache your highlighter instance; Or call \`highlighter.dispose()\` to release unused instances.`);
  let A = !1;
  if (!Q.engine)
    throw new Z("`engine` option is required for synchronous mode");
  const B = (Q.langs || []).flat(1), g = (Q.themes || []).flat(1).map(iB), E = new YC(Q.engine, B), C = new UC(E, g, B, Q.langAlias);
  let I;
  function w(a) {
    G();
    const F = C.getGrammar(typeof a == "string" ? a : a.name);
    if (!F)
      throw new Z(`Language \`${a}\` not found, you may need to load it first`);
    return F;
  }
  function D(a) {
    if (a === "none")
      return { bg: "", fg: "", name: "none", settings: [], type: "dark" };
    G();
    const F = C.getTheme(a);
    if (!F)
      throw new Z(`Theme \`${a}\` not found, you may need to load it first`);
    return F;
  }
  function o(a) {
    G();
    const F = D(a);
    I !== a && (C.setTheme(F), I = a);
    const L = C.getColorMap();
    return {
      theme: F,
      colorMap: L
    };
  }
  function s() {
    return G(), C.getLoadedThemes();
  }
  function e() {
    return G(), C.getLoadedLanguages();
  }
  function i(...a) {
    G(), C.loadLanguages(a.flat(1));
  }
  async function t(...a) {
    return i(await LQ(a));
  }
  function n(...a) {
    G();
    for (const F of a.flat(1))
      C.loadTheme(F);
  }
  async function N(...a) {
    return G(), n(await HQ(a));
  }
  function G() {
    if (A)
      throw new Z("Shiki instance has been disposed");
  }
  function h() {
    A || (A = !0, C.dispose(), gA -= 1);
  }
  return {
    setTheme: o,
    getTheme: D,
    getLanguage: w,
    getLoadedThemes: s,
    getLoadedLanguages: e,
    loadLanguage: t,
    loadLanguageSync: i,
    loadTheme: N,
    loadThemeSync: n,
    dispose: h,
    [Symbol.dispose]: h
  };
}
async function uC(Q) {
  Q.engine || KC("`engine` option is required. Use `createOnigurumaEngine` or `createJavaScriptRegexEngine` to create an engine.");
  const [
    A,
    B,
    g
  ] = await Promise.all([
    HQ(Q.themes || []),
    LQ(Q.langs || []),
    Q.engine
  ]);
  return PC({
    ...Q,
    themes: A,
    langs: B,
    engine: g
  });
}
async function yC(Q) {
  const A = await uC(Q);
  return {
    getLastGrammarState: (...B) => cC(A, ...B),
    codeToTokensBase: (B, g) => eB(A, B, g),
    codeToTokensWithThemes: (B, g) => rQ(A, B, g),
    codeToTokens: (B, g) => KA(A, B, g),
    codeToHast: (B, g) => UA(A, B, g),
    codeToHtml: (B, g) => kC(A, B, g),
    getBundledLanguages: () => ({}),
    getBundledThemes: () => ({}),
    ...A,
    getInternalContext: () => A
  };
}
class tB extends Error {
  constructor(A) {
    super(A), this.name = "ShikiError";
  }
}
function JC() {
  return 2147483648;
}
function SC() {
  return typeof performance < "u" ? performance.now() : Date.now();
}
const pC = (Q, A) => Q + (A - Q % A) % A;
async function fC(Q) {
  let A, B;
  const g = {};
  function E(t) {
    B = t, g.HEAPU8 = new Uint8Array(t), g.HEAPU32 = new Uint32Array(t);
  }
  function C(t, n, N) {
    g.HEAPU8.copyWithin(t, n, n + N);
  }
  function I(t) {
    try {
      return A.grow(t - B.byteLength + 65535 >>> 16), E(A.buffer), 1;
    } catch {
    }
  }
  function w(t) {
    const n = g.HEAPU8.length;
    t = t >>> 0;
    const N = JC();
    if (t > N)
      return !1;
    for (let G = 1; G <= 4; G *= 2) {
      let h = n * (1 + 0.2 / G);
      h = Math.min(h, t + 100663296);
      const a = Math.min(N, pC(Math.max(t, h), 65536));
      if (I(a))
        return !0;
    }
    return !1;
  }
  const D = typeof TextDecoder < "u" ? new TextDecoder("utf8") : void 0;
  function o(t, n, N = 1024) {
    const G = n + N;
    let h = n;
    for (; t[h] && !(h >= G); ) ++h;
    if (h - n > 16 && t.buffer && D)
      return D.decode(t.subarray(n, h));
    let a = "";
    for (; n < h; ) {
      let F = t[n++];
      if (!(F & 128)) {
        a += String.fromCharCode(F);
        continue;
      }
      const L = t[n++] & 63;
      if ((F & 224) === 192) {
        a += String.fromCharCode((F & 31) << 6 | L);
        continue;
      }
      const R = t[n++] & 63;
      if ((F & 240) === 224 ? F = (F & 15) << 12 | L << 6 | R : F = (F & 7) << 18 | L << 12 | R << 6 | t[n++] & 63, F < 65536)
        a += String.fromCharCode(F);
      else {
        const J = F - 65536;
        a += String.fromCharCode(55296 | J >> 10, 56320 | J & 1023);
      }
    }
    return a;
  }
  function s(t, n) {
    return t ? o(g.HEAPU8, t, n) : "";
  }
  const e = {
    emscripten_get_now: SC,
    emscripten_memcpy_big: C,
    emscripten_resize_heap: w,
    fd_write: () => 0
  };
  async function i() {
    const n = await Q({
      env: e,
      wasi_snapshot_preview1: e
    });
    A = n.memory, E(A.buffer), Object.assign(g, n), g.UTF8ToString = s;
  }
  return await i(), g;
}
var dC = Object.defineProperty, OC = (Q, A, B) => A in Q ? dC(Q, A, { enumerable: !0, configurable: !0, writable: !0, value: B }) : Q[A] = B, l = (Q, A, B) => OC(Q, typeof A != "symbol" ? A + "" : A, B);
let Y = null;
function xC(Q) {
  throw new tB(Q.UTF8ToString(Q.getLastOnigError()));
}
class yA {
  constructor(A) {
    l(this, "utf16Length"), l(this, "utf8Length"), l(this, "utf16Value"), l(this, "utf8Value"), l(this, "utf16OffsetToUtf8"), l(this, "utf8OffsetToUtf16");
    const B = A.length, g = yA._utf8ByteLength(A), E = g !== B, C = E ? new Uint32Array(B + 1) : null;
    E && (C[B] = g);
    const I = E ? new Uint32Array(g + 1) : null;
    E && (I[g] = B);
    const w = new Uint8Array(g);
    let D = 0;
    for (let o = 0; o < B; o++) {
      const s = A.charCodeAt(o);
      let e = s, i = !1;
      if (s >= 55296 && s <= 56319 && o + 1 < B) {
        const t = A.charCodeAt(o + 1);
        t >= 56320 && t <= 57343 && (e = (s - 55296 << 10) + 65536 | t - 56320, i = !0);
      }
      E && (C[o] = D, i && (C[o + 1] = D), e <= 127 ? I[D + 0] = o : e <= 2047 ? (I[D + 0] = o, I[D + 1] = o) : e <= 65535 ? (I[D + 0] = o, I[D + 1] = o, I[D + 2] = o) : (I[D + 0] = o, I[D + 1] = o, I[D + 2] = o, I[D + 3] = o)), e <= 127 ? w[D++] = e : e <= 2047 ? (w[D++] = 192 | (e & 1984) >>> 6, w[D++] = 128 | (e & 63) >>> 0) : e <= 65535 ? (w[D++] = 224 | (e & 61440) >>> 12, w[D++] = 128 | (e & 4032) >>> 6, w[D++] = 128 | (e & 63) >>> 0) : (w[D++] = 240 | (e & 1835008) >>> 18, w[D++] = 128 | (e & 258048) >>> 12, w[D++] = 128 | (e & 4032) >>> 6, w[D++] = 128 | (e & 63) >>> 0), i && o++;
    }
    this.utf16Length = B, this.utf8Length = g, this.utf16Value = A, this.utf8Value = w, this.utf16OffsetToUtf8 = C, this.utf8OffsetToUtf16 = I;
  }
  static _utf8ByteLength(A) {
    let B = 0;
    for (let g = 0, E = A.length; g < E; g++) {
      const C = A.charCodeAt(g);
      let I = C, w = !1;
      if (C >= 55296 && C <= 56319 && g + 1 < E) {
        const D = A.charCodeAt(g + 1);
        D >= 56320 && D <= 57343 && (I = (C - 55296 << 10) + 65536 | D - 56320, w = !0);
      }
      I <= 127 ? B += 1 : I <= 2047 ? B += 2 : I <= 65535 ? B += 3 : B += 4, w && g++;
    }
    return B;
  }
  createString(A) {
    const B = A.omalloc(this.utf8Length);
    return A.HEAPU8.set(this.utf8Value, B), B;
  }
}
const JA = class x {
  constructor(A) {
    if (l(this, "id", ++x.LAST_ID), l(this, "_onigBinding"), l(this, "content"), l(this, "utf16Length"), l(this, "utf8Length"), l(this, "utf16OffsetToUtf8"), l(this, "utf8OffsetToUtf16"), l(this, "ptr"), !Y)
      throw new tB("Must invoke loadWasm first.");
    this._onigBinding = Y, this.content = A;
    const B = new yA(A);
    this.utf16Length = B.utf16Length, this.utf8Length = B.utf8Length, this.utf16OffsetToUtf8 = B.utf16OffsetToUtf8, this.utf8OffsetToUtf16 = B.utf8OffsetToUtf16, this.utf8Length < 1e4 && !x._sharedPtrInUse ? (x._sharedPtr || (x._sharedPtr = Y.omalloc(1e4)), x._sharedPtrInUse = !0, Y.HEAPU8.set(B.utf8Value, x._sharedPtr), this.ptr = x._sharedPtr) : this.ptr = B.createString(Y);
  }
  convertUtf8OffsetToUtf16(A) {
    return this.utf8OffsetToUtf16 ? A < 0 ? 0 : A > this.utf8Length ? this.utf16Length : this.utf8OffsetToUtf16[A] : A;
  }
  convertUtf16OffsetToUtf8(A) {
    return this.utf16OffsetToUtf8 ? A < 0 ? 0 : A > this.utf16Length ? this.utf8Length : this.utf16OffsetToUtf8[A] : A;
  }
  dispose() {
    this.ptr === x._sharedPtr ? x._sharedPtrInUse = !1 : this._onigBinding.ofree(this.ptr);
  }
};
l(JA, "LAST_ID", 0);
l(JA, "_sharedPtr", 0);
l(JA, "_sharedPtrInUse", !1);
let RQ = JA;
class mC {
  constructor(A) {
    if (l(this, "_onigBinding"), l(this, "_ptr"), !Y)
      throw new tB("Must invoke loadWasm first.");
    const B = [], g = [];
    for (let w = 0, D = A.length; w < D; w++) {
      const o = new yA(A[w]);
      B[w] = o.createString(Y), g[w] = o.utf8Length;
    }
    const E = Y.omalloc(4 * A.length);
    Y.HEAPU32.set(B, E / 4);
    const C = Y.omalloc(4 * A.length);
    Y.HEAPU32.set(g, C / 4);
    const I = Y.createOnigScanner(E, C, A.length);
    for (let w = 0, D = A.length; w < D; w++)
      Y.ofree(B[w]);
    Y.ofree(C), Y.ofree(E), I === 0 && xC(Y), this._onigBinding = Y, this._ptr = I;
  }
  dispose() {
    this._onigBinding.freeOnigScanner(this._ptr);
  }
  findNextMatchSync(A, B, g) {
    let E = 0;
    if (typeof g == "number" && (E = g), typeof A == "string") {
      A = new RQ(A);
      const C = this._findNextMatchSync(A, B, !1, E);
      return A.dispose(), C;
    }
    return this._findNextMatchSync(A, B, !1, E);
  }
  _findNextMatchSync(A, B, g, E) {
    const C = this._onigBinding, I = C.findNextOnigScannerMatch(this._ptr, A.id, A.ptr, A.utf8Length, A.convertUtf16OffsetToUtf8(B), E);
    if (I === 0)
      return null;
    const w = C.HEAPU32;
    let D = I / 4;
    const o = w[D++], s = w[D++], e = [];
    for (let i = 0; i < s; i++) {
      const t = A.convertUtf8OffsetToUtf16(w[D++]), n = A.convertUtf8OffsetToUtf16(w[D++]);
      e[i] = {
        start: t,
        end: n,
        length: n - t
      };
    }
    return {
      index: o,
      captureIndices: e
    };
  }
}
function qC(Q) {
  return typeof Q.instantiator == "function";
}
function bC(Q) {
  return typeof Q.default == "function";
}
function TC(Q) {
  return typeof Q.data < "u";
}
function WC(Q) {
  return typeof Response < "u" && Q instanceof Response;
}
function vC(Q) {
  var A;
  return typeof ArrayBuffer < "u" && (Q instanceof ArrayBuffer || ArrayBuffer.isView(Q)) || typeof Buffer < "u" && ((A = Buffer.isBuffer) == null ? void 0 : A.call(Buffer, Q)) || typeof SharedArrayBuffer < "u" && Q instanceof SharedArrayBuffer || typeof Uint32Array < "u" && Q instanceof Uint32Array;
}
let aA;
function XC(Q) {
  if (aA)
    return aA;
  async function A() {
    Y = await fC(async (B) => {
      let g = Q;
      return g = await g, typeof g == "function" && (g = await g(B)), typeof g == "function" && (g = await g(B)), qC(g) ? g = await g.instantiator(B) : bC(g) ? g = await g.default(B) : (TC(g) && (g = g.data), WC(g) ? typeof WebAssembly.instantiateStreaming == "function" ? g = await ZC(g)(B) : g = await VC(g)(B) : vC(g) ? g = await WA(g)(B) : g instanceof WebAssembly.Module ? g = await WA(g)(B) : "default" in g && g.default instanceof WebAssembly.Module && (g = await WA(g.default)(B))), "instance" in g && (g = g.instance), "exports" in g && (g = g.exports), g;
    });
  }
  return aA = A(), aA;
}
function WA(Q) {
  return (A) => WebAssembly.instantiate(Q, A);
}
function ZC(Q) {
  return (A) => WebAssembly.instantiateStreaming(Q, A);
}
function VC(Q) {
  return async (A) => {
    const B = await Q.arrayBuffer();
    return WebAssembly.instantiate(B, A);
  };
}
async function zC(Q) {
  return Q && await XC(Q), {
    createScanner(A) {
      return new mC(A.map((B) => typeof B == "string" ? B : B.source));
    },
    createString(A) {
      return new RQ(A);
    }
  };
}
var jC = Uint8Array.from(atob(""), (Q) => Q.charCodeAt(0));
const _C = jC, $C = async (Q) => WebAssembly.instantiate(_C, Q).then((A) => A.instance.exports);
async function AI(Q) {
  const {
    themes: A,
    themeModulePath: B = "@shiki/themes/",
    langModulePath: g = "@shiki/langs/"
  } = Q, E = await Promise.all(
    A.map(
      (w) => import(
        /* @vite-ignore */
        `${B}${w}.mjs`
      )
    )
  ), C = await yC({
    themes: E,
    engine: zC($C)
  });
  async function I(w, D) {
    const { lang: o } = D;
    return o && await C.loadLanguage(
      import(
        /* @vite-ignore */
        `${g}${o}.mjs`
      )
    ), C.codeToHtml(w, D);
  }
  return {
    codeToHtml: I
  };
}
const wI = AI({
  themes: ["vitesse-dark", "vitesse-light"]
});
function BI() {
  let Q = null;
  return async () => (Q || (Q = await import("@shiki/transformers")), Q);
}
const QI = BI();
async function oI(Q) {
  if (Q.length === 0)
    return [];
  const A = await QI();
  return Q.map((B) => {
    const g = `transformer${B.charAt(0).toUpperCase() + B.slice(1)}`;
    return A[g]();
  });
}
function sI(Q) {
  const { copy: A, copied: B } = TQ({ source: Q.code, legacy: !0 }), g = YA(() => SQ(["copy", { copied: B.value }]));
  function E(C) {
    A(Q.code), OB(
      B,
      (I) => {
        I || C.target.blur();
      },
      { once: !0 }
    );
  }
  return {
    copyButtonClick: E,
    btnClasses: g
  };
}
export {
  oI as getTransformers,
  QI as getTransformersModule,
  wI as highlighterTask,
  sI as readyCopyButton
};
